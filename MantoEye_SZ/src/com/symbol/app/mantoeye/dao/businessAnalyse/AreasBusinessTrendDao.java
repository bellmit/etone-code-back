 ...
 */
@Repository
public class AreasBusinessTrendDao extends HibernateQueryDAO {
	...
	public List<CommonFlush> queryAll(int timeLevel,
			String startTime, String endTime, String tableName,
			String areaValue) {
		String sql = buildSql(timeLevel, startTime,endTime, tableName,
				areaValue);
	 ...
	 * @return
	 */
	private String buildSql(int timeLevel,
			String startTime,String endTime, String tableName,
			String areaValue) {
		...
		sql.append(" and viewArea.vcAreaName");
		sql.append(" = '");
		sql.append(areaValue);
		sql.append("'");
		sql.append(" and viewArea.intRaitype = 1");
		sql.append(buildConditionSql(timeLevel, startTime,endTime));

		return sql.toString();
	}
	private String buildConditionSql(int timeLevel,String startTime,String endDate) {
		String sql = "";
		if(timeLevel==3){
			...
			if (sYear != eYear) {
				sql = sql + " and  (( viewArea.intYear = " + sYear + " and viewArea.intWeek>="
						+ sWeek + " and viewArea.intWeek < 100 )" + " or ( viewArea.intYear = "
						+ eYear + " and viewArea.intWeek>0 and viewArea.intWeek <= " + eWeek
						+ " ))  " ;
			} else {
				sql = sql + " and  (( viewArea.intYear = " + sYear + " and viewArea.intWeek>="
						+ sWeek + " and viewArea.intWeek <= " + eWeek + " ))  " ;
			}
		}else{
			sql = " and ddate >= "+ MantoEyeUtils.formatData(startTime, timeLevel)+" and ddate <= "+MantoEyeUtils.formatData(endDate, timeLevel);
		}
		return sql;
	}
	...
	public int getTotalCount(int timeLevel,String startTime, String endTime, String tableName,
			String areaValue) {
		String sql = " select count(*) from ( ";
		sql += buildSql(timeLevel,  startTime,endTime, tableName,
				areaValue);
		sql += " ) as totalCount";
	...
	public List<BussAndBussType> queryBussAndBussType() {
		System.out.println("dao--------------1111111111111");
		String sql = " select nmAreaId as nmBussinessId, 1 as nmBussinessTypeId, convert(varchar,vcAreaName) as vcBussinessName ,'小区集' as vcBussinessTypeName from ftbArea where intType=2 order by nmAreaId asc";
		List list = this.getSession().createSQLQuery(sql).list();
		System.out.println("dao--------------222222222222222");
