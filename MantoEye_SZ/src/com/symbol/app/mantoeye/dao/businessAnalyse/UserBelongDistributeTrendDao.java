...
public class UserBelongDistributeTrendDao extends CommonQueryDAO {
	/**
	 * 分页查询数据
	 * 
	 * @param page
	...
	public Page<CommonFlush> queryByPage(Page page, int timeLevel,
			String startTime, String endTime, String tableName, int userBelongId) {
	...
	public List<CommonFlush> queryAll(int timeLevel, String startTime,
			String endTime, String tableName, int userBelongId) {
		String sql = buildSql(timeLevel, startTime, endTime, tableName,
				userBelongId);
	...
	private String buildSql(int timeLevel, String startTime, String endTime,
			String tableName, int userBelongId) {
		...
		sql.append(" and viewArea.intUserbelongId=");
		sql.append(userBelongId);
		sql.append(" and viewArea.intRaitype = 1");
		sql.append(buildConditionSql(timeLevel, startTime, endTime));
		// sql.append(Common
		// .buildDateTimeCompareSql(startDate, endDate, timeLevel));
	...
	public int getTotalCount(int timeLevel, String startTime, String endTime,
		...
		sql += buildSql(timeLevel, startTime, endTime, tableName, userBelongId);
		sql += " ) as totalCount";
	...
	public List<UserBelongBean> getAllUserBelongIdAndDesc() {
		String sql = "select t.intUserBelongId userBelongId,t.vcDesc 'desc' from dtbUserBelong t";

		List<UserBelongBean> list = this.getSession().createSQLQuery(sql)
				.addScalar("userBelongId", Hibernate.INTEGER).addScalar("desc",
						Hibernate.STRING).setResultTransformer(
						Transformers.aliasToBean(UserBelongBean.class)).list();
