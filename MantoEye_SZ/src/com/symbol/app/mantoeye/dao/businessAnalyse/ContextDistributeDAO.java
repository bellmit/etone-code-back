...
@Repository
public class ContextDistributeDAO extends HibernateDao {
	...
	public void initMap() {
		mapBussType.clear();
		mapBuss.clear();
		mapBussType.put(4, "vStatMonthGprsContentType_V2");
		mapBussType.put(3, "vStatWeekGprsContentType_V2");
		mapBussType.put(2, "vStatDayGprsContentType_V2");
		mapBussType.put(1, "ftbStatHourGprsConTentType_V2");
		mapBuss.put(4, "vStatMonthGprsContent_V2");
		mapBuss.put(3, "vStatWeekGprsContent_V2");
		mapBuss.put(2, "vStatDayGprsContent_V2");
		mapBuss.put(1, "ftbStatHourGprsConTent_V2");
		mapContext.put(1, "ftbStatHourGprsConDetails_V2");
		mapContext.put(2, "vStatDayGprsConDetails_V2");
		mapContext.put(3, "vStatWeekGprsConDetails_V2");
		mapContext.put(4, "vStatMonthGprsConDetails_V2");
	}
	...
	public Page<CommonFlush> queryBussDistributePage(Page<CommonFlush> page,
		...
		if (businessType.equals("business")) {
			sql.append("select vcGroupName,nmAppUpFlush,nmAppDownFlush,nmFlush,nmUsers,nmVisitCounts,nmFlush/nmUsers as nmAveFlush,nmLocalFlush,nmLocalUsers from ");
			sql.append(mapBuss.get(timeLevel));
			sql.append(" where 1=1 ");
			sumFlushSql = sumFlushSql + mapBuss.get(timeLevel);
			sumFlushSql = sumFlushSql + " where 1=1 ";
			if (business != null && business != "") {
				// sumFlushSql = sumFlushSql + "and vcBussinessName in ('"
				// + business.replaceAll(",", "','") + "') ";
				sql.append(" and vcGroupName in ('");
				sql.append(business.replaceAll(",", "','"));
				sql.append("')");
			}
		} else {
			sql.append("select vcParentGroupName,nmAppUpFlush,nmAppDownFlush,nmFlush,nmUsers,nmVisitCounts,nmFlush/nmUsers as nmAveFlush,nmLocalFlush,nmLocalUsers from ");
		...
		while (it.hasNext()) {
			...
			if (businessType.equals("business")) {// 业务
				...
				commonFlush.setLocaltotalFlush(Common.StringToLong(obj[7] + ""));	//本地总流量 V3
				commonFlush.setLocalintImsis(Common.StringToLong(obj[8] + ""));	//本地用户数 V3
	...
	public List<CommonFlush> queryBusinessDistribute(int timeLevel,
		...
		while (it.hasNext()) {
			...
			if (businessType.equals("business")) {// 业务
				...
				commonFlush.setVisit(Common.StringToLong(obj[5] + ""));
				commonFlush.setNmAveFlush(Common.StringTodouble(obj[6] + ""));
				commonFlush.setFlushRate(CommonUtils.formatPercent(Common.StringToLong(obj[3] + ""), sumFlush));
				commonFlush.setLocaltotalFlush(Common.StringToLong(obj[7] + ""));	//本地总流量 V3
				commonFlush.setLocalintImsis(Common.StringToLong(obj[8] + ""));	//本地用户数 V3
				commonFlush.calculateData();
				commonFlush.calculateFlushRate(dbs[0], dbs[1]);
	...
	public List<CommonFlush> queryBusinessDateByType(int dataType,
			int timeLevel, String date, String businessType) {// 根据业务类型查询具体业务分布
		// 计算占比
		Double[] dbs = findSumTotalData(timeLevel, date);
		...
		if (businessType.equals("其他"))
			sql = new StringBuffer(
					"select top 10 vcGroupName,nmFlush,nmUsers,nmVisitCounts,nmAppUpFlush,nmAppDownFlush,nmFlush/nmUsers as nmAveFlush,nmLocalFlush,nmLocalUsers from ");
		else {
			sql = new StringBuffer(
					"select vcGroupName,nmFlush,nmUsers,nmVisitCounts,nmAppUpFlush,nmAppDownFlush,nmFlush/nmUsers as nmAveFlush,nmLocalFlush,nmLocalUsers from ");
		}
		sql.append(mapBuss.get(timeLevel));
		sql.append(" where 1=1");
		sql.append(this.getQueryCondition(dataType, timeLevel, dateStr));
	...
	public String buildSql(Page page, int isTD, int areaType, int timeLevel,
		...
		case CommonConstants.MANTOEYE_SPACE_LEVEL_BSC:
			...
			table = (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_HOUR) ? "vStatHourBscContentType_V2"
					: (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_DAY) ? "vStatDayBscContentType_V2"
							: (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_WEEK) ? "vStatWeekBscContentType_V2"
									: "vStatMonthBscContentType_V2";
			break;
		case CommonConstants.MANTOEYE_SPACE_LEVEL_SGSN:
			fields = " intSgsnId,vcName, ";
			table = (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_HOUR) ? "vStatHourSgsnContentType_V2"
					: (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_DAY) ? "vStatDaySgsnContentType_V2"
							: (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_WEEK) ? "vStatWeekSgsnContentType_V2"
									: "vStatMonthSgsnContentType_V2";
			break;
		case CommonConstants.MANTOEYE_SPACE_LEVEL_STREET:
			fields = " intStreetId,vcName, ";
			table = (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_HOUR) ? "vStatHourStreetContentType_V2"
					: (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_DAY) ? "vStatDayStreetContentType_V2"
							: (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_WEEK) ? "vStatWeekStreetContentType_V2"
									: "vStatMonthStreetContentType_V2";
			break;
		case CommonConstants.MANTOEYE_SPACE_LEVEL_SALEAREA:
			fields = " intSaleAreaId,vcSaleAreaName, ";
			table = (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_HOUR) ? "vStatHourSaleAreaContentType_V2"
					: (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_DAY) ? "vStatDaySaleAreaContentType_V2"
							: (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_WEEK) ? "vStatWeekSaleAreaContentType_V2"
									: "vStatMonthSaleAreaContentType_V2";
			break;
		case CommonConstants.MANTOEYE_SPACE_LEVEL_BRANCH:
			fields = " intBranchId,vcBranchName, ";
			table = (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_HOUR) ? "vStatHourCompanyContentType_V2"
					: (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_DAY) ? "vStatDayCompanyContentType_V2"
							: (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_WEEK) ? "vStatWeekCompanyContentType_V2"
									: "vStatMonthCompanyContentType_V2";
			break;
		}
		sql = " select " + fields;
	...
	public List<BussAndBussType> queryBussAndBussType() {
		//String sql = "select b.nmBussinessId,b.nmBussinessTypeId,b.vcBussinessName,t.vcBussinessTypeName from dtbBusinessSite b join dtbBusinessType t on b.nmBussinessTypeId=t.nmBussinessTypeId";
		String sql = "select nmGroupId as nmBussinessId,nmParentId as nmBussinessTypeId,vcGroupName as vcBussinessName,vcParentGroupName as vcBussinessTypeName from dtbGroupTree where nmTypeId=2  group by nmParentId,vcParentGroupName,nmGroupId,vcGroupName order by nmBussinessTypeId";
	...
	public List<TopFlushBean> queryTop10Busi(Date date, String type) {
		...
		String sql = "Select top 10 vcBussinessName,nmAppUpFlush,nmAppDownFlush,nmFlush,nmUsers,nmVisitCounts,nmAveFlush from vStatDayGprsBuss "
	...
	public TopFlushBean queryTop10Busi2(Date date, String type) {
		...
		int day = CommonUtils.getDay(date);
		List<TopFlushBean> result = new ArrayList<TopFlushBean>();
		String sql = "Select top 1 vcBussinessName,nmAppUpFlush,nmAppDownFlush,nmFlush,nmUsers,nmVisitCounts,nmAveFlush from vStatDayGprsBuss "
				+ " where intRaitype=1  and intYear = "
				+ year
				+ " and intMonth = " + month + " and intDay= " + day;
		if ("wap".equals(type)) {
	...
	public int isExistLeaf(String groupName) {
		String sql = "select count(*) from tbProductDimensGroup a,tbProductDimensGroup b where a.parent=b.GroupId and b.GroupName='"+groupName+"'";
