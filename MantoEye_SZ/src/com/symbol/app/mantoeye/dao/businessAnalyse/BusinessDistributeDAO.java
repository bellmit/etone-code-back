...
@Repository
public class BusinessDistributeDAO extends HibernateDao {
	...
	public void initMap() {
		mapBussType.clear();
		mapBuss.clear();
		mapBussType.put(4, "vStatMonthGprsBussType");
		mapBussType.put(3, "vStatWeekGprsBussType");
		mapBussType.put(2, "vStatDayGprsBussType");
		mapBussType.put(1, "vStatHourGprsBussType");
		mapBuss.put(4, "vStatMonthGprsBuss");
		mapBuss.put(3, "vStatWeekGprsBuss");
		mapBuss.put(2, "vStatDayGprsBuss");
		mapBuss.put(1, "vStatHourGprsBuss");
	}
	...
	public Page<CommonFlush> queryBussDistributePage(Page<CommonFlush> page,
		...
		if (businessType.equals("business")) {
			sql.append("select vcBussinessName,nmAppUpFlush,nmAppDownFlush,nmFlush,nmUsers,nmVisitCounts,nmAveFlush from ");
			...
			if (business != null && business != "") {
				...
				// + business.replaceAll(",", "','") + "') ";
				sql.append(" and vcBussinessName in ('");
				sql.append(business.replaceAll(",", "','"));
				sql.append("')");
			}
		} else {
			sql.append("select vcBussinessTypeName,nmAppUpFlush,nmAppDownFlush,nmFlush,nmUsers,nmVisitCounts,nmAveFlush from ");
	...
	public List<CommonFlush> queryBusinessDateByType(int dataType,
		...
		if (businessType.equals("其他"))
			sql = new StringBuffer(
					"select top 10 vcBussinessName,nmFlush,nmUsers,nmVisitCounts,nmAppUpFlush,nmAppDownFlush,nmAveFlush from ");
		else {
			sql = new StringBuffer(
					"select vcBussinessName,nmFlush,nmUsers,nmVisitCounts,nmAppUpFlush,nmAppDownFlush,nmAveFlush from ");
		}
		sql.append(mapBuss.get(timeLevel));
		sql.append(" where 1=1");
		sql.append(this.getQueryCondition(dataType, timeLevel, dateStr));
		...
		if (timeLevel == 1) {
			sql.append(" and vcBussinessTypeName='");
			sql.append(businessType);
			sql.append("'");
		} else {
			sql.append(" and nmBussinessId in(");
			sql.append(" select aa.nmBussinessId from dtbBusinessSite aa,dtbBusinessType bb where aa.nmBussinessTypeId = bb.nmBussinessTypeId and bb.vcBussinessTypeName = '");
			sql.append(businessType);
			sql.append("') ");
		}
		sql.append(" order by nmFlush desc");
	...
	public String buildSql(Page page, int isTD, int areaType, int timeLevel,
		...
		case CommonConstants.MANTOEYE_SPACE_LEVEL_BSC:
			...
			table = (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_HOUR) ? "vStatHourBscBussType"
					: (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_DAY) ? "vStatDayBscBussType"
							: (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_WEEK) ? "vStatWeekBscBussType"
									: "vStatMonthBscBussType";
			break;
		case CommonConstants.MANTOEYE_SPACE_LEVEL_SGSN:
			fields = " intSgsnId,vcName, ";
			table = (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_HOUR) ? "vStatHourSgsnBussType"
					: (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_DAY) ? "vStatDaySgsnBussType"
							: (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_WEEK) ? "vStatWeekSgsnBussType"
									: "vStatMonthSgsnBussType";
			break;
		case CommonConstants.MANTOEYE_SPACE_LEVEL_STREET:
			fields = " intStreetId,vcName, ";
			table = (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_HOUR) ? "vStatHourStreetBussType"
					: (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_DAY) ? "vStatDayStreetBussType"
							: (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_WEEK) ? "vStatWeekStreetBussType"
									: "vStatMonthStreetBussType";
			break;
		case CommonConstants.MANTOEYE_SPACE_LEVEL_SALEAREA:
			fields = " intSaleAreaId,vcSaleAreaName, ";
			table = (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_HOUR) ? "vStatHourSaleAreaBussType"
					: (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_DAY) ? "vStatDaySaleAreaBussType"
							: (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_WEEK) ? "vStatWeekSaleAreaBussType"
									: "vStatMonthSaleAreaBussType";
			break;
		case CommonConstants.MANTOEYE_SPACE_LEVEL_BRANCH:
			fields = " intBranchId,vcBranchName, ";
			table = (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_HOUR) ? "vStatHourCompanyBussType"
					: (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_DAY) ? "vStatDayCompanyBussType"
							: (timeLevel == CommonConstants.MANTOEYE_TIME_LEVEL_WEEK) ? "vStatWeekCompanyBussType"
		...
		sql = sql + " and vcBussinessName = '" + business + "'";
...
//			sql += " and intRaitype=1";
			sql += " and (intRaitype = 1 or intRaitype is null)";	//原来默认是查GPRS1类型数据，现分时段查全网数据	V3
			sql += " order by ddate";
		} else {
			sql = " select nmFlush,nmUsers,";
...
//					+ endTime + "' and intRaitype=1 order by ddate";
					+ endTime + "' and (intRaitype = 1 or intRaitype is null) order by ddate";	//默认查全文数据	V3
		}
