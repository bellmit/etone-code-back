...
@Repository
public class TerminalTaskDAO extends HibernateDao {
	...
	public void saveTerminalTask(TerminalTaskEntity terminalTask) {
		...
		// String
		// queryId="select nmTerminalPolicyId from ftbTerminalPolicy where vcPolicyName='"+terminalTask.getTaskName()+",";//获取当前插入任务ID
		Integer taskId = this.queryIdByTaskName(terminalTask.getTaskName());

		logger.info("**" + taskId);

		// String bussPath = PropertiesUtil.getInstance().getProperty(
		// "buss.filePath");//在wcmp_config.properties配置文件路径
		String terminalPath = PropertiesUtil.getInstance().getProperty(
				"termina.filePath");

		wirterAndLoadTableBuss(terminalPath, terminalTask, taskId);
		wirterAndLoadTableTerminal(terminalPath, terminalTask, taskId);
	...
	public void wirterAndLoadTableBuss(String bussPath,
		...
		for (int i = 0; i < businessTypeList.size(); i++) {
			...
			if (i == 0) {
				sql = sql + "(bt.nmBussinessTypeId=" + bussType
		...
		/*
		 * Long date=new Date().getTime(); String bussName=date+"bussName.txt";
		 * PrintWriter bussPW=null; try { bussPW=new PrintWriter(new
		 * File(bussPath+bussName)); List<BussAndBussType>
		 * listBuss=terminalTask.getListBuss();
		 * if(listBuss!=null&&!listBuss.isEmpty()){ for(int
		 * i=0;i<listBuss.size();i++){ BussAndBussType buss=listBuss.get(i);
		 * String
		 * str=taskId+","+buss.getBusinessTypeId()+","+buss.getBusinessId();
		 * bussPW.println(str); bussPW.flush(); } }
		 * 
		 * } catch (FileNotFoundException e) { logger.error(e.getMessage());
		 * }finally{ bussPW.close(); }
		 * 
		 * String columns="nmTerminalPolicyId,nmBussinessTypeId,nmBussinessId";
		 * this.getSession().createSQLQuery(this.bulidString(
		 * "ftbTerminalPolicyMapBusiness", columns,
		 * bussPath+bussName)).executeUpdate(); new File(bussName).delete();
		 */
	 ...
	 * @param terminalTask
	 * @param taskId
	 *            批量insert into 到终端关系表中
	 */
	public void wirterAndLoadTableTerminal(String terminalPath,
			TerminalTaskEntity terminalTask, Integer taskId) {
		Map<String, String> map = terminalTask.getListTerminal();
		String brandId = "";
		String typeId = "";
		String sql = "insert into ftbTerminalPolicyMapTerm(nmTerminalPolicyId,intTerminalId,intTmerTypeId) select tp.nmTerminalPolicyId,tb.intTerminalId,tt.intTmerTypeId  from dtbTerminalType tt,dtbTerminalBrand tb,ftbTerminalPolicy tp"
				+ " where 1=1";

		List<String> brandList = new ArrayList<String>(map.keySet());
		sql = sql + " and (";
		for (int i = 0; i < brandList.size(); i++) {
			brandId = brandList.get(i);
			typeId = map.get(brandId);
			if (i == 0) {
				sql = sql + " (tb.intTerminalId=" + brandId
						+ " and tt.intTmerTypeId in(" + typeId + ")) ";
			} else {
				sql = sql + " or (tb.intTerminalId=" + brandId
						+ " and tt.intTmerTypeId in(" + typeId + ")) ";
			}
		}
		sql = sql + ") and tp.nmTerminalPolicyId=" + taskId;
		...
		int num = this.getSession().createSQLQuery(sql).executeUpdate();
		logger.info("****************insert msisdn number:"+num);

		/*
		 * Long date=new Date().getTime(); String
		 * terminalName=date+"terminalName.txt"; PrintWriter terminalPW=null;
		 * try { terminalPW=new PrintWriter(new
		 * File(terminalPath+terminalName)); List<TerminalEntity>
		 * listTerminal=terminalTask.getListTerminal();
		 * if(listTerminal!=null&&!listTerminal.isEmpty()){ for(int
		 * i=0;i<listTerminal.size();i++){ TerminalEntity
		 * terminal=listTerminal.get(i); String
		 * str=taskId+","+terminal.getBradId()+","+terminal.getTypeId();
		 * terminalPW.println(str); terminalPW.flush(); } } } catch
		 * (FileNotFoundException e) { logger.error(e.getMessage()); }finally{
		 * terminalPW.close(); }
		 * 
		 * String columns2="nmTerminalPolicyId,intTmerTypeId,intTerminalId";
		 * this
		 * .getSession().createSQLQuery(this.bulidString("ftbTerminalPolicyMapTerm"
		 * , columns2, terminalPath+terminalName)).executeUpdate(); new
		 * File(terminalName).delete();
		 */
	}
	...
	public void deleteTask(String keys) {
		...
		String sqlBuss = "delete from ftbTerminalPolicyMapBusiness where nmTerminalPolicyId in ("
				+ keys + ")";
		// String
		// sqlResult="delete from ftbStatWeekTermBuss where nmTerminalPolicyId in ("+keys+")";
		...
		// this.getSession().createSQLQuery(sqlResult).executeUpdate();
