...
public class TerminalTaskAction extends BaseDispatchAction {
	...
	public void query() {
		...
		taskEntity.setTaskStatus(status);
		 ...
		 this.page=terminalTaskManager.queryEntity(page, taskEntity);
	...
	public String checkUnique() {
		...
		if(oldTaskName.equals(taskName)){
			...
			int i=terminalTaskManager.isTaskName(taskName);
			if(i==-1){//-1表示不存在该任务名称
				Struts2Utils.renderText("true");
			}else{
	...
	public String  saveTask() throws Exception{
		...
		String terminal=request.getParameter("terminalIds");
		...
		taskEntity.setTaskName(taskName);
		taskEntity.setListTerminal(this.handlerTerminal(terminal));
		...
		try{
			...
			msg = "添加终端任务成功!";
			terminalTaskManager.insertTask(taskEntity);
	 ...
	 * @return
	 */
	public Map<String,String> handlerTerminal(String terminal){
		Map<String,String> map=new HashMap<String,String>();
		String[] terminals=terminal.split(",");
		logger.info("&&&"+terminals.length);
		for(int i=0;i<terminals.length;i++){
			TerminalEntity tEntity=new TerminalEntity();
			String[] str=terminals[i].split(":");
			if(str.length==3){
				String brandId=str[0].toString();
				String typeId=str[1].toString();
				if(map.get(brandId)==null){
					map.put(brandId, typeId);
				}else{
					
					String typeIds=map.get(brandId)+","+typeId;
					map.put(brandId, typeIds);
				}
			}
		}
	...
	public void deleteTask(){
		...
		try{
			...
			msg = "删除终端任务成功!";
			terminalTaskManager.deleteTask(taskId);
			commonManagerImpl.log(request, msg);
			Struts2Utils.renderText(msg);
		}catch(Exception e){
			logger.error(e.getMessage());
...
//					tableBean.setElement8(ts.getBusinessTypeName());
	...
	public String edit() throws Exception{
		...
		TerminalTaskEntity taskEntity=terminalTaskManager.queryTaskByTaskId(Common.StringToInt(id));
		String terminal=baseTerminalManager.bulidBrandAndTypeByTaskId(Common.StringToInt(id));
		...
		taskEntity.setTerminalNames(terminals[1]);
		taskEntity.setBrandAndType(terminals[0]);
