...
import com.symbol.app.mantoeye.service.blackberry.BlackBerryAreaYysManager;
...
public class BlackBerryAreaYysAction extends BaseDispatchAction {

	@Autowired
	private BlackBerryAreaYysManager blackBerryAreaYysManager;
	...
	public void query() throws ServletException, IOException {
		...
		//
		//		
		// page = blackBerryAreaYysManager.query(page, dataType,
		...
		page.setOrderBy("nmTotalFlush");
		page.setOrder("desc");
		list = blackBerryAreaYysManager.listData(page, dataType, Common
				.StringToInt(spaceLevel_search), Common
				.StringToInt(timeLevel_search), time_search, yysName);
		String sxml = buildChartStr(list);
		this.getServletRequest().getSession().setAttribute(
				SessionConstants.SESSION_DATA_BB_LDC, sxml);
	...
	public void getAjaxChartXmlData() throws Exception {
		String xml = this.getServletRequest().getSession().getAttribute(
				SessionConstants.SESSION_DATA_BB_LDC)
				+ "";
	...
	//
	//		
	// List<CommonFlush> list = blackBerryAreaYysManager.listData(page,dataType,
	// Common.StringToInt(spaceLevel_search),
	// Common.StringToInt(timeLevel_search), time_search,yysName);
	//		
	//		
	// int chaertType = Common.StringToInt(spaceLevel_search);
	// String name = "";
	...
	// // + chartTitle
	// // +
	// " trip=\"流量用户数统计{br}"+name+"：{name}{br}总流量：{total}MB{br}上行流量：{up}MB{br}下行流量：{down}MB{br}用户数：{imsis}\">";
	// // if (list != null && !list.isEmpty()) {
	// // for (CommonFlush c : list) {
	// // xml = xml + "<Statdata name=\"" + c.getBusinessName()
	...
	public void export() throws ServletException, IOException {
		yysName = new String(yysName.getBytes("ISO-8859-1"), "UTF-8");
		GridServerHandler gridServerHandler = new GridServerHandler(
				Struts2Utils.getRequest(), Struts2Utils.getResponse());
		SortInfo si = gridServerHandler.getSingleSortInfo();
		if (si != null) {
		...
		List<CommonFlush> list = blackBerryAreaYysManager.listData(page,
				dataType, Common.StringToInt(spaceLevel_search), Common
						.StringToInt(timeLevel_search), time_search, yysName);
		String exportmsg = ExportMsg.EXPORT_BLACKBREEY_LACSPACE
	...
	public void getAllYysName() {
		String allYysName = blackBerryAreaYysManager.getAllYysName();
		Struts2Utils.renderText(allYysName);
	}
