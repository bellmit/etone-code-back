...
<table style="overflow: hidden;">
	<tr>
		...
		<td id="td12"
			style="width: 49%; height: 49%; border: 1px solid #8DB2E3; padding-bottom: -1px;">
		<table>
			<tr>
				<td>
				<div id="data_container" style="width: 100%; height: 80%;"></div>
				</td>
			</tr>
		</table>
		</td>
...
var dsConfig = {
	...
	var gridConfig = {
	    ...
	    remoteSort : true ,
	    pageSizeList: [10,15,20,25,50,100],
	...
	function initGridInfo2(datagrid) {
		var width = $('#td22').width()-6; 
		//var height = $('#td12').height()-46;
		var height = $('#td22').height()-6;
		datagrid.setSize(width, height);//设置列表的宽高
     	width = width - 160;
     //	 $('#datagrid_headDiv').css('display','none');
     	 for (var j in datagrid.columnMap) {
			 if( 'dtDate' == j){
				datagrid.getColumn(j).setWidth(80);
			 }else if( 'vcTitle' == j){
				 datagrid.getColumn(j).setWidth(width);
			 } 
		}
     	 
     	$('#datagrid_headDiv').empty();
		 
		width = width + 66;
		   $('#datagrid_headDiv').html('<div style="float: left;width: 10000px;">'+
		    	    '  <table id="datagrid_headTable" class="gt-head-table" cellspacing="0" cellpadding="0" border="0">'+
		    	    ' <tbody>'+
		    		'      <tr class="gt-hd-row">'+
		    		'	       <td style="width:'+width+'px;">'+
		    		'		       <div class="gt-inner gt-inner-center" unselectable="on" title="公告标题">'+
		    		'		          <span>公告标题</span>'+
		    		'			       <div class="gt-hd-tool">'+
		    		'			          <span class="gt-hd-icon"></span>'+
		    		'				       <span class="gt-hd-button"></span>'+
		    		'				       <span class="gt-hd-split" style="cursor: col-resize; "></span>'+
		    		'			       </div>'+
		    		'				 </div>'+
		    		'			</td>'+
		    		'            <td>'+
		    		'		          <span style="line-height:1.9em;font-weight: bold;color: #333333;">发布时间</span>'+
		    		'</td>'+
		    		'	   </tr>'+
		    		'   </tbody>'+
		    	   '</table>'+
		       '</div>'+
		    '<div id="datagrid_columnMoveS" class="gt-column-moveflag"></div>'+
		    '<div id="datagrid_headerGhost" class="gt-head-ghost"></div>');
	}
</script>
<script type="text/javascript">
...
jQuery("#list20").jqGrid({
		url:'/indexNote_queryJson.do?1=1',
		datatype : 'json', 
		data: maindata,
		height : 192,
		width : 492,
		...
		gridComplete:function(){
				...
				if(x=='1280'){
					...
					jQuery("#list20").setGridWidth(680);
				}		
		},
		rowNum:6,
   		rowList:[2,6],
