 (function(c) {
    ...
    function a(g, k) {
        ...
        i,
        h,
        e = k.expose && c.tools.expose.version;//e为版本号
        var f = k.target || g.attr("rel");//k 为页面传入对象
        i = f ? c(f) : null || g;//i 为制定传入对象DIV 
        if (!i.length) {//
            throw "Could not find Overlay: " + f
        }
        //g 为所传入的 a连接 图片jquery对象
        if (g && g.index(i) == -1) {
            g.click(function(p) {
                o.load(p);
        ...
        c.extend(o, {
            load: function(u) {//event 对象
                ...
                if (k.oneInstance) {//d 为0，1，2，3，4，5
                    c.each(d, 
                    function() {
                        this.close(u)
                    })
                ...
                if (k.closeOnClick) {//点击屏幕任何地方都相应时间关闭窗口
            ...
            getContent: function() {//i 为制定传入对象DIV ID
                return i
            },
            getOverlay: function() {
                return i
            },
            getTrigger: function() {//i装载图片DIV 所有 a 对象
                return g
            },
            getClosers: function() {//close 图片对象
                return j
            },
            isOpened: function() {
                return h
    ...
    c.fn.overlay = function(e) {//e{target: '#gallery',// optional exposing effect  expose: '#f1f1f1'}
 ...
 (function(b) {
    ...
    b.fn.gallery = function(d) {
        ...
        m = this.overlay();
        var r = this,
        j = m.getOverlay(),//显示DIV对象
        k = j.find(o.next),//  向后
        g = j.find(o.prev),//向前 
        e = j.find(o.info),//信息栏
        c = j.find(o.progress),//进度条
        h = g.add(k).add(e).css({
            opacity: o.opacity
        }),
        s = m.getClosers(),//关闭图片
        ...
        function p(u) {
            ...
            s.hide();
            var t = u.attr("href");
            t='/skin/Default/images/MantoEye/3276760045_742ebd4663.jpg';
            var v = new Image();
            v.onload = function() {
                c.fadeOut();
                var y = b("#" + o.imgId, j);
                if (!y.length) {
                    y = b("<img/>").attr("id", o.imgId).css("visibility", "hidden");//添加图片到大DIV中
                    j.prepend(y)
                }
               ...
               // var divid=r.attr("id");
              //   l=parseInt(divid.substring(divid.length-1,divid.length));
                l = r.index(u);//查找当前点击为第几个图片
