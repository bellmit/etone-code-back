...
(function () {
	...
	var ct = /\(\?[:=!]|\[[^\]]+\]/g;
	var cu = /\(/g;
	var D = z.extend({constructor:function (a) {
		this[Z] = [];
		this.merge(a);
	}, exec:function (g) {
		var h = this, p = this[Z];
		...
		return "(" + this[Z].join(")|(") + ")";
	}}, {IGNORE:"$0", Item:z.extend({constructor:function (a, b) {
	...
	var bH = D.extend({ignoreCase:true});
	var cw = /\x01(\d+)/g, cx = /'/g, cy = /^\x01/, cz = /\\([\da-fA-F]{1,4})/g;
	var bp = [];
	var cA = new bH({"<!\\-\\-|\\-\\->":"", "\\/\\*[^*]*\\*+([^\\/][^*]*\\*+)*\\/":"", "@(namespace|import)[^;\\n]+[;\\n]":"", "'(\\\\.|[^'\\\\])*'":bJ, "\"(\\\\.|[^\"\\\\])*\"":bJ, "\\s+":" "});
	function cB(a) {
		return cA.exec(a);
	}
	function bI(c) {
	...
	var cD = new D({Width:"Height", width:"height", Left:"Top", left:"top", Right:"Bottom", right:"bottom", onX:"onY"});
	function A(a) {
		return cD.exec(a);
	}
	var bK = [];
	...
	var br = "ie7-";
	var bM = z.extend({constructor:function () {
		this.fixes = [];
		this.recalcs = [];
	}, init:Q});
	var bs = [];
	...
	var bd = k < 6;
	var bO = /^(href|src)$/;
	var bu = {"class":"className", "for":"htmlFor"};
	IE7._5 = 1;
	IE7._e = function (a, b) {
		var c = a.all[b] || null;
		if (!c || c.id == b) {
	...
	var bR = D.extend({constructor:function (a) {
		this.base(a);
		this.sorter = new D;
	...
	var bS = {"":"%1!=null", "=":"%1=='%2'", "~=":/(^| )%1( |$)/, "|=":/^%1(-|$)/, "^=":/^%1/, "$=":/%1$/, "*=":/%1/};
	var bT = {"first-child":"!IE7._b(e%1)", "link":"e%1.currentStyle['ie7-link']=='link'", "visited":"e%1.currentStyle['ie7-link']=='visited'"};
	...
	var S;
	var be = {};
	var T = new bR({" (\\*|[\\w-]+)#([\\w-]+)":function (a, b, c) {
		o = false;
	...
	IE7.CSS = new (bM.extend({parser:new bH, screen:"", print:"", styles:[], rules:[], pseudoClasses:k < 7 ? "first\\-child" : "", dynamicPseudoClasses:{toString:function () {
		var a = [];
		for (var b in this) {
			a.push(b);
		}
	...
	var bw = z.extend({constructor:function (a) {
		this.media = a;
		this.load();
		IE7.CSS[a] = this;
		IE7.CSS.refresh();
	...
	var bf;
	var n = IE7.Rule = z.extend({constructor:function (a, b) {
	...
	var L = n.extend({constructor:function (a, b, c, d, f) {
		this.attach = b || "*";
	...
	var cd = z.extend({constructor:function (a, b) {
		this.name = a;
		this.apply = b;
		this.instances = {};
	...
	IE7.HTML = new (bM.extend({fixed:{}, init:Q, addFix:function () {
	...
	new function (_) {
		...
		function collapseMargin(a, b) {
			if (!a.runtimeStyle[b]) {
				...
				if (c && IE7.hasLayout(c) && !IE7[b == "marginTop" ? "_b" : "_a"](a)) {
					return;
				}
		...
		var fixWidth = function (g) {
			...
			function l(a) {
				...
				if (a.currentStyle.minWidth != "none" && c <= q(a, a.currentStyle.minWidth)) {
					...
					if (a.currentStyle.maxWidth != "none" && c >= q(a, a.currentStyle.maxWidth)) {
	...
	if (k >= 5.5 && k < 7) {
		...
		IE7.HTML.addRecalc("img,input", function (a) {
			...
			w(a, "onpropertychange", function () {
				if (!bj && event.propertyName == "src" && a.src.indexOf(bg) == -1) {
					by(a);
				}
			});
		});
	...
	var bk = {backgroundColor:"transparent", backgroundImage:"none", backgroundPositionX:null, backgroundPositionY:null, backgroundRepeat:null, borderTopWidth:0, borderRightWidth:0, borderBottomWidth:0, borderLeftStyle:"none", borderTopStyle:"none", borderRightStyle:"none", borderBottomStyle:"none", borderLeftWidth:0, height:null, marginTop:0, marginBottom:0, marginRight:0, marginLeft:0, width:"100%"};
