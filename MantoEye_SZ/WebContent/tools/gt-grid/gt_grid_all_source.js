...
GT.Const = GT.Const || {};
GTConst = GT.Const;
GT.Const.Grid = {COL_CLASS_PREFIX:"td.", DEFAULT_ECG_ID:"gt", SHADOW_ROW:"_shadowRow", HIDE_HEADER_ROW:"_hideListRow", COL_T_CLASSNAME:"gt-col-", SKIN_CLASSNAME_PREFIX:"gt-skin-", SCROLLBAR_WIDTH:18, MIN_COLWIDTH:40, AJAX_HEADER:["isAjaxRequest", "true"]};
...
GT.$byId = function ($, A) {
	...
	if (GT.U.contains(["object", "embed"], !$.tagName ? $.tagName.toLowerCase() : "")) {
		return $;
	}
	return GT.$e($);
};
...
GT.Utils = {P_START:"@{", P_END:"}", P_VAR_NAME:"obj_in", parseExpression:function (ex, pName, argNames, pStart, pEnd) {
	...
	while ($ && (B--) > 0) {
		...
		if (GT.U.hasClass($.className, "gt-grid") && A != "div") {
			break;
		}
		$ = $.parentNode;
	}
	...
	if ($ && !GT.U.hasClass($, _)) {
		$.className = GT.U.gu5($.className + " " + _);
	}
	return $;
}, removeClass:function ($, _) {
	if ($) {
		$.className = GT.U.gu5($.className.replace(new RegExp("(^|\\s)" + _ + "(?:\\s|$)"), "$1"));
	}
	return $;
...
GT.Ajax = GT.$class({ck2:function () {
	...
	var D;
	if (this.queryParameters && GT.$type(this.queryParameters, "object")) {
		D = GT.toQueryString(this.queryParameters);
	} else {
		if (GT.$type(this.queryParameters, "string")) {
			D = this.queryParameters;
		}
	}
	if (D && GT.$type(C, "string")) {
		C = C + "&" + D;
	}
	if (E == "post") {
		var _ = H.indexOf("?");
...
GT.DataSetDefault = {SEQUENCE:0, uniqueField:GT.Const.DataSet.SN_FIELD, recordType:"object", recordXpath:null, dataXML:null, currentBegin:0, cursor:0, startRecordNo:0, cacheData:false, cacheModifiedData:true, modified:false, ck2:function () {
	return {fields:[], fieldsName:[], fieldsMap:{}, fieldsInfo:{}, data:null, Ge6:[], sf2:null, additional:[], sortInfo:[], queryInfo:[], reocrdIndex:{}, updatedRecords:{}, B$6:{}, updatedFields:{}, insertedRecords:{}, deletedRecords:{}, vv8:GT.$empty, xV0:GT.$empty};
}, SO5:function ($) {
	GT.$extend(this, $);
	...
	this.data = this.data || [];
	var $ = this, D = GT.Const.DataSet.SN_FIELD;
	...
	var B = $[GT.Const.DataSet.SN_FIELD], A = $[this.unfieldIdx], E = this.fieldsMap[F].type, _ = this.fieldsMap[F].index, D;
	...
	if ($) {
		var A = $[GT.Const.DataSet.SN_FIELD], _ = $[this.unfieldIdx];
...
GT.$extend(GT.Template, {Grid:{main:function (_) {
	...
	var cellColor = "cell-bgcolor1 ";
	if ($.colIndex ==0) {
		cellColor = "cell-bgcolor1 ";
	} else if($.colIndex ==1){
		cellColor = "cell-bgcolor2 ";
	}else if($.colIndex ==2){
		cellColor = "cell-bgcolor3 ";
	}else{
		var t = parseInt(($.colIndex-3)/GT.isDisplay);
		if(t%2==0){
			cellColor = "cell-bgcolor4 ";
		}else{
			cellColor = "cell-bgcolor5 ";
		}
	}
	return ["<div class=\"gt-inner " +cellColor + ($.align ? " gt-inner-" + $.align + " " : "") + "", "\" >"].join("");
	...
	var F = C + D.Ut2, $ = C + D.gc3, E = "style=\"width:" + $ + "px;\"";
	_ = _ || "&#160;";
	...
	var $ = C.domId, A = C.gridId, B = GT.$chk(C.mu8) ? C.mu8 : true, _ = C.title || "Dialog";
...
GT.GridDefault = {id:GT.Const.Grid.DEFAULT_ECG_ID, defaultColumnWidth:70, domList:["gridWaiting", "separateLine", "gridMask", "gridGhost", "gridFormTextarea", "gridFormFileName", "gridFormExportType", "gridForm", "gridIFrame", "activeDialog", "gridDialog", "gridDialogTitle", "gridDialogBody", "gridFilterRSCache", "titleBar", "toolTipDiv", "freezeHeadTable", "freezeHeadDiv", "freezeBodyDiv", "freezeView", "resizeButton", "pageSizeSelect", "pageStateBar", "toolBar", "Ml9", "columnMoveS", "headerGhost", "headFirstRow", "bodyFirstRow", "headTable", "headDiv", "bodyDiv", "gridDiv", "viewport", "container"], defaultConst:["action", "recordType", "exportType", "exportFileName", "exception", "parameters", "queryParameters", "data", "pageInfo", "filterInfo", "sortInfo", "columnInfo", "fieldsName", "insertedRecords", "updatedRecords", "updatedFields", "deletedRecords", "success", "succeedData", "failedData", "remotePaging", "remoteSort", "remoteFilter", "remoteGroup"], language:"default", skin:"default", dataRoot:null, dataPageInfo:null, dataException:null, formid:null, isNative:false, loadURL:null, saveURL:null, exportURL:null, exportType:null, exportFileName:null, sortInfo:null, editable:true, resizable:false, showGridMenu:false, showIndexColumn:false, otherDataInfo:null,allowCustomSkin:false, allowFreeze:false, allowHide:false, allowGroup:false, allowResizeColumn:true, simpleScrollbar:true, scrollbarClass:null, monitorResize:false, readOnly:false, stripeRows:true, lightOverRow:true, evenRowCss:"gt-row-even", clickStartEdit:true, remotePaging:true, remoteSort:false, remoteFilter:false, remoteGroup:false, autoLoad:true, submitColumnInfo:true, autoUpdateSortState:true, autoUpdateEditState:true, autoUpdateGroupState:true, autoUpdateFreezeState:true, autoSelectFirstRow:true, autoEditNext:false, submitUpdatedFields:false, autoSaveOnNav:false, reloadAfterSave:true, recountAfterSave:true, recount:false, showEditTool:true, showAddTool:true, showDelTool:true, showSaveTool:true, showReloadTool:true, showPrintTool:true, showFilterTool:true, showChartTool:true, showPageState:true, transparentMask:false, justShowFiltered:true, toolbarPosition:"bottom", toolbarContent:"nav | goto | pagesize | reload | add del save | print | filter chart | state", width:"100%", height:"100%", minWidth:50, minHeight:50, dataRoot:"data", custom2Cookie:true, multiSort:false, multiGroup:false, multiSelect:true, selectRowByCheck:false, html2pdf:true, GTGridPath:"../../gt-grid", ck2:function () {
	...
	GT.GridNum++;
	var F = {}, E = this;
	GT.$each(this.domList, function ($, _) {
		E[$] = null;
	});
	...
	if (this.toolCreator) {
		for (var G in this.toolCreator) {
			GT.ToolFactroy.register(G, this.toolCreator[G]);
		}
	}
	...
	delete H.pageInfo, H.pageSize, this.pageInfo, this.pageSize;
	this.navigator = new GT.Navigator({gridId:this.id, pageInfo:B});
}, legacy:function () {
	var $ = this;
	...
	var G = this.columns.length, E = 0, C = true, B = {};
	for (var F = 0; F < G; F++) {
		var A = this.columns[F];
		A.grid = this;
		A.gridId = this.id;
		if (A.isCheckColumn) {
			A = GT.Grid.ar6(this, A);
		}
		...
		if (_.editor && _.editor.AO9()) {
			this.gridEditorCache.appendChild(_.editor.AO9());
		}
		if (_.renderer == "by editor" && _.editor) {
			...
			if (GT.$type(_.renderer, "string")) {
				_.renderer = GT.U.parseExpression(_.renderer, "record", "value,record,col,grid,colNo,rowNo");
			}
		}
	...
	if (this.separateLine) {
		this.separateLine.style.top = this.el7 + "px";
	}
	...
	var C = this.Ml0, _ = this.freezeBodyDiv.clientWidth, $ = this.activeCell.offsetLeft + ((GT.isFF2 || GT.isFF1) ? 0 : C), B = $ + this.activeCell.offsetWidth, F = this.activeCell.offsetTop, D = F + this.activeCell.offsetHeight, A = this.bodyDiv.scrollLeft, E = this.bodyDiv.scrollTop, J = A + this.bodyDiv.clientWidth + C, G = E + this.bodyDiv.clientHeight;
	if (GT.$chk(I)) {
	...
	if (F == GT.Const.Key.ESC) {
		...
		if (F == GT.Const.Key.ENTER) {
			var G = GT.U.getEventTarget(D);
			if (this.editing && GT.U.getTagName(G) == "TEXTAREA") {
				return;
			}
			GT.U.stopEvent(D);
			if (this.editing) {
			...
			if (this.editing && F == GT.Const.Key.TAB && D.shiftKey) {
				$ = this.Gk4(A);
				B($);
			} else {
				if (this.editing && F == GT.Const.Key.TAB) {
					$ = this.nW3(A);
					B($);
				} else {
					if (A && !this.editing) {
	...
	var A = this.activeCell, _ = this.bodyDiv, C = this.Ml0;
	...
	if (this.activeEditor && this.activeEditor instanceof GT.Dialog) {
		...
		_.appendChild(this.activeEditor.AO9());
	}
	...
	if (this.activeEditor && this.activeEditor instanceof GT.Dialog) {
		...
		this.gridEditorCache.appendChild(this.activeEditor.AO9());
	}
	...
	var _ = B, A = B.rowIndex, $ = !_.id ? this.gridTbodyList[0] : this.freezeBodyTable.tBodies[0];
	row2 = $ ? $.rows[A] : null;
	if (!row2 && $ && $.parentNode.tFoot) {
		row2 = $.parentNode.tFoot.rows[A - $.rows.length];
	}
	return _.id ? [_, row2, A] : [row2, _, A];
}, QC1:function (_, E) {
	var A = _, D = GT.U.getCellIndex(_), $ = null, B = 0, C = _.parentNode;
	E = E || this.Yi4(C);
	if (E[1] == C) {
	...
	if (!C || GT.U.hasClass(C.cells[0], "gt-nodata-cell")) {
		return;
	}
	...
	var F = C.parentNode, E = $.eventTarget, B = (GT.U.getTagName(E) == "INPUT" && E.className == "gt-f-check");
	if (B) {
		GT.cf6(E, this);
	}
	this.bo0(C, A);
	...
	if (!GT.U.hasClass(C, "gt-index-col")) {
		this.Mr2(A, C, D);
	}
	this.ry2(null, C);
}, clickCount:0, clickInterval:500, Se5:function (A, B, _) {
	var C = this;
	if (_ == C.bodyDiv) {
		if (B == "scroll") {
			C.gb7();
	...
	var A = this;
	GT.U.addEvent($, _, function (B) {
		A.Se5(B, _, $);
	});
}, getEventTargets:function (A, _) {
	_ = _ || GT.U.getEventTarget(A);
	var F = null, G = -1, E = -1, D = null, $ = null, C = null, B = GT.U.LG4("td", _, A);
	if (B) {
		F = B.parentNode;
	...
	GT.gO9();
	if (_.monitorResize) {
		GT.U.addEvent(window, "resize", function ($) {
			_.Ci9();
		});
		_.hasResizeListener = true;
	}
	GT.U.addEvent(_.gridDiv, "mousedown", function ($) {
		_.gb7();
	});
	...
	GT.U.addEvent(_.headDiv, "selectstart", function ($) {
		GT.U.stopEvent($);
		return false;
	});
	...
	function $($) {
		...
		if (A) {
			GT.U.addClass(A, "gt-hd-row-over");
		}
		if (_.lastOverHdCell != A) {
			GT.U.removeClass(_.lastOverHdCell, "gt-hd-row-over");
		}
		_.lastOverHdCell = A;
	}
	...
	if (!this.toolTipDiv) {
		this.toolTipDiv = GT.$e("div", {className:"gt-cell-tooltip gt-breakline"});
	...
	for (var B = 0; B < this.columnList.length; B++) {
		var A = this.columnList[B], _ = {id:A.id, header:A.header || A.title, fieldName:A.fieldName, fieldIndex:A.fieldIndex, sortOrder:A.sortOrder, hidden:A.hidden, exportable:A.exportable, printable:A.printable, exportnumber:A.exportnumber||false};
		$.push(_);
	}
	return $;
	...
	var E = this.saveURL, I = this.getInsertedRecords(), J = this.getUpdatedRecords(), $ = this.getDeletedRecords(), A = (I != null && I.length > 0 || J != null && J.length > 0 || $ != null && $.length > 0);
	if (!N && !A) {
		...
		if (GT.$invoke(this, "beforeSave", [B, this]) !== false) {
			this.showWaiting();
			var M = this;
			return this.Sr3(E, B, "json", function ($) {
				return function (_) {
					M.cM1(_, B, $);
				};
			}(N), function (A, _) {
	...
	if (GT.$invoke(this, "beforeLoad", [_, this]) !== false) {
		...
		this.showWaiting();
		var $ = this.Sr3(E, _, "json", function ($) {
			C.GH0($, _);
		}, function (A, _) {
			var $ = {};
	...
	if (GT.$type(_, "array")) {
		...
		if (_[this.CONST.recordType]) {
			this.dataset.Up3(_[this.CONST.recordType]);
		}
		if (_[this.CONST.pageInfo]) {
			GT.$extend($, _[this.CONST.pageInfo]);
		}
	...
	if (!response || GT.$type(response, "string", "number")) {
	...
	var A = _[this.CONST.exception] || ($ ? ($.message || "") : "");
       // alert(" LOAD Failed! " + "n Exception : n" + A)
}, saveFailure:function (_, $) {
	var A = _[this.CONST.exception] || ($ ? ($.message || "") : "");
       // alert(" SAVE Failed! " + "n Exception : n" + A)
}, exportFailure:function (_, $) {
	var A = _[this.CONST.exception] || ($ ? ($.message || "") : "");
      //  alert(" Export " + _.type + " ( " + _.fileName + " ) Failed! " + "n Exception : n" + A)
	...
	var A, C, D = this.colNum, _ = GT.T_G.rowStart(this, this.rowNum);
	if (!this.gridTable.tFoot) {
		this.gridTable.appendChild(GT.$e("tfoot"));
	}
	if (!this.freezeBodyTable.tFoot) {
		this.freezeBodyTable.appendChild(GT.$e("tfoot"));
	}
	A = GT.U.createTrFromHTML(this.MP1(_, $, this.rowNum, D), this.gridTable.tFoot);
	if (this.showIndexColumn) {
		C = GT.U.createTrFromHTML(this.Xa1(_, $, this.rowNum, D), this.freezeBodyTable.tFoot);
	} else {
	...
	var $ = this.mo8().length > 0, _ = !!(this.gridTable.tFoot && this.gridTable.tFoot.rows.length > 0);
	return $ || _;
}, R_1:function () {
	if (!this.Rf2()) {
		var _ = ["<tr class=\"gt-row gt-row-empty\" >"];
		for (var $ = 0; $ < this.colNum; $++) {
			_.push(GT.T_G.cell(this.columnList[$], "&#160;"));
		}
	...
	for (var $ = _.rows.length - 1; $ >= 0; $--) {
		GT.U.removeNodeTree(_.rows[$]);
	}
}, tD1:function (E, I, _) {
	var A = ("" + E.rowEnd).length;
	A = (A < 2 ? 1.5 : A) * 7 + 2 + 1;
	var G = A + this.Ut2, $ = A + this.gc3, J = "style=\"width:" + G + "px;\"", F = "style=\"width:" + $ + "px;\"";
	this.indexColumnCell = ["<td class=\"gt-index-col\" " + J + " ><div class=\"gt-inner\" " + F + " >", "</div></td>"];
	if (E.showIndexColumn) {
		E.Ml0 = A;
		E.cleanFreezeHead();
		var B = GT.$e("tr", {className:"gt-hd-row"}), H = GT.T_G.xm4(E, A, null);
		B.appendChild(H);
	...
	var E = [A];
	for (var B = 0; B < G; B++) {
		var _ = this.columnList[B], D = C && C[_.id] ? C[_.id].attr : null;
	...
	var F = this.colNum, M = C - J, D, B, N = 1, A = {}, _ = null, N = 1, G = [];
	for (var K = 0; K < F; K++) {
	...
	for (var E = 0; E < this.sortInfo.length; E++) {
		...
		if (B) {
			var $ = this.columnMap[B.columnId], A = B.sortOrder || "defaultsort";
			$.sortOrder = A;
	...
	if (GT.U.isNumber($)) {
		...
		if (GT.U.getTagName($) == "TD") {
			...
			if (GT.U.getTagName($) == "TR") {
				return this.getRecordByRow($);
			} else {
				if (GT.$type($, "object") && !$.tagName) {
					return $;
				} else {
					if (this.selectedRows.length > 0) {
						_ = this.selectedRows[this.selectedRows.length - 1].getAttribute(GT.Const.DataSet.INDEX) / 1;
					} else {
						_ = 0;
					}
				}
	...
	for (var E in this.dataset.updatedRecords) {
		var A = this.dataset.updatedRecords[E], G = this.dataset.B$6[E], F = this.getRowByRecord(A);
		if (F) {
	...
	for (E in this.dataset.deletedRecords) {
		A = this.dataset.deletedRecords[E], F = this.getRowByRecord(A);
		this.del(F);
	}
}, filterGrid:function ($) {
	...
	if (this.resizable && (this.toolbarPosition == "bottom" || this.toolbarPosition == "b") && this.Ml9) {
		this.resizeButton = GT.$e("div", {id:this.id + "_resizeButton", className:"gt-tool-resize", innerHTML:"&#160;"});
		this.resizeButton.setAttribute("unselectable", "on");
		this.Ml9.appendChild(this.resizeButton);
		var D = this;
		GT.U.addEvent(this.resizeButton, "mousedown", function ($) {
			...
			if (A == "|") {
				...
				if (A == "state" || A == "info" || A == "pagestate") {
					if (!this.pageStateBar) {
						this.pageStateBar = GT.ToolFactroy.create(this, "pagestate", this.showPageState);
					}
					if (E != C.length - 1) {
	...
	if (!this.showGridMenu || !this.Ml9 || !this.toolBar) {
		return;
	}
	var F = this, _ = F.id;
	this.gridMenuButton = new GT.Button({gridId:this.id, parentItem:this, container:this.toolBar, cls:"gt-tool-gridmenu", NA8:true});
	var F = this, C = !this.allowGroup ? null : GT.lr2("group", {gridId:_, checkValid:function ($) {
		return $.grouped;
	}, checkFn:"group", uncheckFn:"ungroup", checkType:F.multiGroup ? "checkbox" : "radio", canCheck:function ($) {
		return !$.hidden;
	}}), E = !this.allowFreeze ? null : GT.lr2("freeze", {gridId:_, checkValid:function ($) {
		return $.frozen;
	}, checkFn:"freeze", uncheckFn:"unfreeze", canCheck:function ($) {
		return !$.hidden;
	}}), H = !this.allowHide ? null : GT.lr2("show", {gridId:_, checkValid:function ($) {
		return !$.hidden;
	...
	if (this.allowCustomSkin) {
		$ = new GT.MenuItem({gridId:this.id, type:"", text:this.getMsg("CHANGE_SKIN"), cls:"gt-icon-skin"});
		var G = [];
		for (var I = 0; I < this.skinList.length; I++) {
			G.push(new GT.MenuItem({gridId:this.id, type:"radiobox", text:this.skinList[I].text, checked:I === 0, onclick:A(this.skinList[I].value)}));
		}
		$.ae8(G, "R");
	}
	this.gridMenuButton.ae8([$, C ? new GT.MenuItem({gridId:F.id, type:"", text:F.getMsg("MENU_GROUP_COL"), cls:"gt-icon-groupcol", onclick:function () {
		B(C, F.getMsg("MENU_GROUP_COL"));
	}}) : null, E ? new GT.MenuItem({gridId:F.id, type:"", text:F.getMsg("MENU_FREEZE_COL"), cls:"gt-icon-freeze", onclick:function () {
		B(E, F.getMsg("MENU_FREEZE_COL"));
	}}) : null, H ? new GT.MenuItem({gridId:F.id, type:"", text:F.getMsg("MENU_SHOW_COL"), cls:"gt-icon-hidecol", onclick:function () {
		B(H, F.getMsg("MENU_SHOW_COL"));
	}}) : null, new GT.MenuItem({gridId:this.id, type:"", text:GT_GRID_VER})], D);
}, showMask:function ($) {
	if ($ || this.transparentMask) {
		GT.U.addClass(this.gridMask, "gt-transparent");
	} else {
		GT.U.removeClass(this.gridMask, "gt-transparent");
	}
	...
	switch (D) {
	  case "filter":
		A.filterDialog = A.filterDialog || GT.Vc8({title:A.getMsg("DIAG_TITLE_FILTER"), gridId:A.id});
		A.filterDialog.show();
		break;
	  case "chart":
		...
		var C = [], B = "", $ = 300, E = 300;
		if (_) {
			A.charDialog = A.charDialog || new GT.Dialog({gridId:A.id, container:A.gridMask, id:"charDialog", width:$, height:E, autoRerender:true, title:A.getMsg("DIAG_TITLE_CHART")});
			A.charDialog.show();
			A.chart = new GT.Chart({swfPath:A.GTGridPath + "/flashchart/fusioncharts/charts/", width:$ - 3, height:E - 23, container:A.charDialog.bodyDiv});
			GT.$each(A.columnList, function ($, A) {
				if ($.chartCaption) {
...
setTotalRowNum:function ($) {
	...
	var $ = this;
	GT.$each(this.selectedRows, function (_) {
	...
	var N = this, H = N.colNum, M = N.getGroupInfo(K, G), _ = " >", J = lastRecord = null, I = 0, E = "";
	for (var B = K; B < G; B++) {
		J = N.dataset.getRecord(B);
		if (!J) {
			break;
		}
		var $ = N.customRowAttribute(J, B, N) || "", A = GT.T_G.rowStartS(N, B, $);
		if (!F) {
			E = N.rowKey + N.getUniqueField(J);
			J[GT.Const.DataSet.ROW_KEY] = E;
			N.currentRowNum++;
			if (N.showIndexColumn) {
				C.push(N.Xa1(A + _, J, B, H));
			}
		} else {
			if (B > 0 && D > 0 && (B % D) == 0) {
				O.push("\n<!-- gt : page separator  -->\n");
		...
		if (N.isNextGroup(J, lastRecord, B)) {
			O.push(N.yR9(A + _, J, B, H));
		}
		var L = M[I++];
	...
	var _ = this, $ = [];
	GT.$each(_.columnList, function (_, B) {
	...
	E = E.join("\n");
	var D = A.HD7(), _ = GT.doc.activeElement;
	function B($) {
		$.writeln("<style>");
		$.writeln(E);
		$.writeln("</style>");
	...
	if ($.gridRowList.length == 0) {
		GT.$each($.gridTbodyList, function (_) {
			$.cn2(_.rows);
		});
	}
	return $.gridRowList;
	...
	grid = this;
	var $ = false, E = this.columnList.length, B = A !== true && this.freezeHeadDiv.style.display == "block" ? this.frozenColumnList.length : 0;
	F = F < B ? B : F;
	if (F >= E) {
		F = E - 1;
		F ^= _;
	...
	for (var C = 0; C < D.length; C++) {
		GT.U.insertNodeBefore(D[C].cells[_], D[C].cells[F]);
	}
	GT.U.moveItem(this.columnList, _, F);
	for (C = 0; C < E; C++) {
		this.columnList[C].colIndex = C;
	}
}, bo8:function () {
	...
	var A = this;
	GT.$each(this.gridTbodyList, function (_, $) {
		GT.U.removeNode(_);
...
GT.$extend(GT.Grid, {isSelectedRow:function ($) {
	...
	var A = GT.U.parseInt(B.gridGhost.style.width) + "px", $ = GT.U.parseInt(B.gridGhost.style.height) + "px";
	...
	C.closeGridMenu();
	var B = GT.U.getPageX(A), D = $.headCell.offsetLeft;
	...
	if (G.separateLine.style.display == "block") {
		...
		if (!G.customHead && G.headerGhost.style.display == "block") {
	...
	if (E.separateLine.style.display == "block") {
		...
		$.Nf9 = D - E.UQ0[0];
		var C = $.Nf9 - $.ml3, B = C + parseInt($.width);
		$.setWidth(B);
		E.gi4 = -1;
		...
		if (!E.customHead && E.headerGhost.style.display == "block") {
			var F = GT.isIE ? A.x : A.pageX, G = E.columnMoveS.getAttribute("newColIndex"), _ = E.headerGhost.getAttribute("colIndex");
	...
	for (var B = 0; B < A.length; B++) {
		if (A[B].indexOf(GT.Const.Grid.SKIN_CLASSNAME_PREFIX) === 0) {
			A[B] = "";
		}
	}
	...
	if (!_) {
		_ = GT.$chk(E.fieldIndex) ? "record[\"" + E.fieldIndex + "\"];" : "grid.getUniqueField(record);";
	}
	if (typeof _ == "string") {
...
GT.$extend(GT.Grid, {render:function ($) {
	$ = GT.$grid($);
	return function () {
		$.render();
	};
	...
	A = A || GT.Grid.tK4($, C);
	var _ = GT.U.nextElement(A), D = GT.U.nextElement(_);
	...
	GT.U.addEvent(C, "mousedown", function (_) {
		B.gb7();
		if (B.endEdit() === false) {
			return;
		}
	...
	GT.U.addEvent(C, "click", function (A) {
		var D = GT.U.getEventTarget(A);
		if (!B.Vl4) {
			GT.$invoke(B, "onHeadClick", [A, C, $, B]);
			if (GT.U.getTagName(D) == "INPUT" && D.type == "checkbox" && GT.U.hasClass(D, "gt-f-totalcheck")) {
				GT.gh9(D, B, $);
			} else {
				if ($.sortable && D.className != "gt-hd-button") {
					...
					B.sorting = true;
					/*edit by rankin*/
					//var _ = $.sortOrder == "asc" ? "desc" : ($.sortOrder == "desc" ? "defaultsort" : "asc"), E = B.TG7($);					
					var _ = "desc",
                    E = B.TG7($);
                    if(B.sortInfo!=null){
	...
	if ($.resizable) {
		...
		GT.U.addEvent(D, "mousedown", function (_) {
			B.gb7();
			GT.U.stopEvent(_);
	...
	while ((B = B.parentNode)) {
		if (GT.U.getTagName(B) == _ && GT.U.hasClass(B, A)) {
			$ = B.id;
			break;
		}
	}
...
var Ext = Ext || null;
(Ext && Ext.reg) && (Ext.reg("gtgrid", GT.Grid));
GT.cf6 = function (_, C, $) {
	C = GT.$grid(C);
	_ = GT.$(_);
	if (_.checked == $) {
	...
	var B = GT.U.LG4("td", _), D = B.parentNode, A = C.Yi4(D)[0];
	if ($ === true || $ === false) {
		_.checked = $;
	}
	if (_.checked) {
...
GT.gh9 = function (H, J, G, I) {
	...
	var _ = GT.U.LG4("td", H), $ = GT.U.getCellIndex(_), K = H.checked, E = G.frozen ? J.getAllFreezeRows() : J.getAllRows();
...
GT.ColumnDefault = {CLASS_PREFIX:".", destroyList:["sortIcon", "hdTool", "menuButton", "separator", "frozenSortIcon", "frozenHdTool", "frozenHeadCell", "headCell", "firstCell"], id:0, fieldName:null, width:120, minWidth:45, header:null, styleClass:null, align:"left", headAlign:"left", emptyText:"", sortable:true, resizable:true, moveable:true, editable:true, hideable:true, frozenable:true, groupable:true, filterable:true, printable:true, exportable:true, sortOrder:null, hidden:false, frozen:false, toolTip:false, beforEdit:null, afterEdit:null, renderer:function (A, $, _, B, D, C) {
	...
	this.editor = null;
	GT.$each(this.destroyList, function ($, _) {
	...
	if (_ && C === 0) {
		...
		if (!GT.isIE) {
			GT.Grid.Qe7(A, this, this.frozenHeadCell, this.frozenSortIcon);
		}
	}
}, freeze:function (B) {
	...
	var F = A.headTable.tBodies[0].rows, _ = A.freezeHeadTable.tBodies[0].rows, $, D, C = 10;
	if (!A.Sa2) {
		$ = GT.T_G.xm4(A, C, null);
		D = GT.T_G.freezeBodyCell(A, C, null);
	}
	for (var E = 0; E < F.length; E++) {
		this.HP4(F[E], A.freezeHeadTable.tBodies[0], _[E], E, G, $, A, true);
	}
	if (A.rowNum < 1) {
	}
	A.uB0 = false;
	...
	for (E = 0; E < F.length; E++) {
		this.HP4(F[E], A.freezeBodyTable.tBodies[0], _[E], E, G, D, A);
	}
	if (!B) {
	...
	if (!A.Sa2) {
		A.freezeHeadDiv.style.left = A.freezeBodyDiv.style.left = 0 - (C + A.Ut2) + A.freezeFixW + "px";
	}
	A.Sa2 = true;
	A.syncScroll();
	GT.U.removeNode($, D);
...
GT.Navigator = GT.$class({ck2:function () {
	return {pageInfo:{pageSize:20, pageNum:1, totalRowNum:0, totalPageNum:1, startRowNum:0, endRowNum:0}};
}, inited:false, SO5:function (_) {
	var $ = _.pageInfo || {};
	delete _.pageInfo;
	GT.$extend(this, _);
	GT.$extend(this.pageInfo, $);
}, destroy:function () {
	var $ = this, _ = ["firstPageButton", "prevPageButton", "nextPageButton", "qM2", "gotoPageButton"];
	GT.$each(_, function (_, A) {
		if ($[_] && $[_].destroy) {
			$[_].destroy();
		}
		$[_] = null;
	...
	this.firstPageButton = new GT.Button({container:$.toolBar, cls:"gt-first-page", onclick:this.sy4, onclickArgs:[this]});
	this.prevPageButton = new GT.Button({container:$.toolBar, cls:"gt-prev-page", onclick:this.Ae7, onclickArgs:[this]});
	this.nextPageButton = new GT.Button({container:$.toolBar, cls:"gt-next-page", onclick:this.Nk5, onclickArgs:[this]});
	this.qM2 = new GT.Button({container:$.toolBar, cls:"gt-last-page", onclick:this.ba2, onclickArgs:[this]});
	this.inited = true;
	if (!$.loading) {
		this.Md9();
	}
	...
	this.gotoPageButton = new GT.Button({container:B.toolBar, cls:"gt-goto-page", onclick:this.dc5, onclickArgs:[this], text:B.getMsg("GOTOPAGE_BUTTON_TEXT")});
	if (B.toolBar) {
		var _, A;
		this.pageInput = GT.$e("input", {type:"text", className:"gt-page-input"});
		var $ = this;
		GT.U.addEvent(this.pageInput, "keydown", function (_) {
			var A = _.keyCode;
		...
		_ = GT.$e("div", {innerHTML:B.getMsg("PAGE_BEFORE"), className:"gt-toolbar-text"}), A = GT.$e("div", {innerHTML:B.getMsg("PAGE_AFTER"), className:"gt-toolbar-text"});
	...
	var A = this.pageInfo = $ || this.pageInfo, _ = GT.$grid(this.gridId);
	if (this.inited) {
		if (this.pageInput) {
	...
	if (_ && _.pageSizeSelect) {
		...
		GT.U.createSelect(GT.U.listToMap(_.pageSizeList), this.pageInfo.pageSize, {}, _.pageSizeSelect);
	}
}, Rn5:function (A, C, $) {
	A = A || this;
	var B = A.pageInfo.pageNum, _ = GT.$grid(A.gridId);
...
GT.BaseMenuItem = GT.$class({id:null, gridId:null, cls:null, type:null, onclickArgs:null, parentItem:null, reference:null, container:null, text:null, toolTip:null, itemBox:null, itemIcon:null, itemText:null, itemAfterIcon:null, subMenu:null, SO5:function ($) {
	this.disabled = false;
	this.NA8 = false;
	...
	if ($.parentItem) {
		($.parentItem.DC0) && $.parentItem.DC0(B);
		if ($.parentItem.currenItem) {
			GT.U.removeClass($.parentItem.currenItem.itemBox, "gt-menu-activemenu");
		}
	...
	if (_) {
		if (!this.subMenu) {
			this.subMenu = new GT.GridMenu({gridId:this.gridId, parentItem:this, reference:this.itemBox});
...
GT.Button = GT.BaseMenuItem.extend({SO5:function (_) {
	...
	this.itemBox = GT.$e("a", {href:"javascript:void(0);return false;", className:this.className, title:this.toolTip});
	this.itemIcon = GT.$e("div", {className:this.cls});
	this.itemBox.appendChild(this.itemIcon);
	this.container.appendChild(this.itemBox);
	if (this.NA8) {
		GT.Button.bd6(this.container);
	}
	var $ = this;
	GT.U.addEvent($.itemBox, "mousedown", function (_) {
		if (!$.disabled) {
			GT.U.addClass($.itemBox, $.clickClassName);
		}
	});
	GT.U.addEvent($.itemBox, "mouseup", function (_) {
		if (!$.disabled) {
			GT.U.removeClass($.itemBox, $.clickClassName);
		}
	});
	GT.U.addEvent($.itemBox, "click", function (_) {
		$.Xd8(_, $);
	});
	GT.U.addEvent($.itemBox, "dblclick", function (_) {
		$.Xd8(_, $);
	});
}});
GT.$extend(GT.Button, {bd6:function (_) {
	var $ = GT.$e("div", {className:"gt-image-button gt-button-split"});
	if (_) {
		_.appendChild($);
	}
	return $;
}, createCommonButton:function (D, _, A, $, C, B) {
	return new GT.Button({id:D, container:C, cls:_, onclick:A, onclickArgs:$, NA8:B});
}, re8:function (_) {
	var $ = GT.$e("div", {className:"gt-toolbar-comp"});
	if (_) {
		if (GT.$type(_, "string", "number")) {
			$.innerHTML = _;
		} else {
			$.appendChild(_);
		}
...
GT.MenuItem = GT.BaseMenuItem.extend({SO5:function (_) {
	this.vP7(_);
	if (this.type == "checkbox" || this.type == "radiobox") {
		this.cls = this.checked ? ("gt-icon-" + this.type) : "gt-icon-unchecked";
	}
	this.itemBox = GT.$e("a", {href:"javascript:void(0);return false;", className:"gt-menuitem"});
	this.itemIcon = GT.$e("div", {className:"gt-menu-icon " + this.cls});
	this.itemText = GT.$e("div", {className:"gt-checkboxtext", innerHTML:this.text, title:this.toolTip});
	this.itemAfterIcon = GT.$e("div", {className:"gt-aftericon " + this.afterIconClassName});
	this.itemBox.appendChild(this.itemIcon);
...
GT.$extend(GT.MenuItem, {bd6:function (_) {
	var $ = GT.$e("div", {className:"gt-image-button gt-button-split"});
	if (_) {
		_.appendChild($);
	}
...
GT.GridMenu = GT.$class({gridId:null, parentItem:null, container:null, Xl3:null, aE5:null, destroy:function () {
	this.container = null;
	this.parentItem = null;
	this.itemList;
	GT.$each(this.itemList, function (_, A, $) {
		GT.U.removeNode(_);
		$[A] = null;
	});
}, SO5:function (_) {
	...
	this.position = "";
	GT.$extend(this, _);
	this.menuBox = GT.$e("div", {className:this.className, style:{display:"none", left:"10px", top:"10px"}});
	var $ = GT.$grid(this.gridId) || {};
	...
	var B, _, A = GT.U.getXY(this.reference, this.container);
	B = A[0];
	_ = A[1];
	this.Xl3 = this.Xl3 || 0;
	this.aE5 = this.aE5 || 0;
...
GT.ToolFactroy = {register:function (_, $) {
	...
	if (C && GT.$type(C, "function")) {
		C = C(D, E, _);
	} else {
		if (C) {
			var A = C.name || E, B = C.onclick || C.action;
			C = new GT.Button({container:C.container || D.toolBar, cls:C.cls || "gt-tool-" + A, toolTip:C.toolTip || D.getMsg("TOOL_" + A.toUpperCase()), onclick:function ($) {
				B($, D);
			}});
		}
	}
	...
	A.pageSizeSelect = _;
	function $(B) {
		A.setPageInfo({pageSize:_.value / 1});
	...
	text1 = GT.$e("div", {innerHTML:A.getMsg("PAGESIZE_BEFORE"), className:"gt-toolbar-text"});
	text2 = GT.$e("div", {innerHTML:A.getMsg("PAGESIZE_AFTER"), className:"gt-toolbar-text"});
	...
	var $ = GT.$e("div", {innerHTML:"&#160;", className:"gt-page-state"});
...
GT.Widget = GT.$class({id:null, dom:null, setDom:function ($) {
	this.dom = $;
}, AO9:function () {
	return this.dom;
}, show:function () {
...
GT.DialogDefault = {mu8:true, autoRerender:true, title:null, body:null, buttonZone:null, headHeight:20, hidden:false, SO5:function ($) {
	if ($) {
		GT.$extend(this, $);
	}
	this.domId = (this.gridId ? this.gridId + "_" : "") + this.id;
	this.buttonLayout = this.buttonLayout || "h";
	this.dialogId = this.id, GT.WidgetCache[this.id] = this;
}, destroy:function () {
	this.container = null;
	...
	this.container = $ || this.container;
	if (!this.rendered) {
		this.dom = this.dom || GT.$e("div", {className:"gt-grid-dialog"});
	...
	this.setSize();
	this.Au8();
	if (GT.$type(this.valueDom, "function")) {
	...
	if (GT.$type(this.body, "function")) {
		this.body = this.body(_);
	}
	if (!this.body) {
	} else {
		if (GT.$type(this.body, "string")) {
	...
	if (_.length > 0) {
		this.buttonZone = this.buttonZone || GT.$e("div", {className:"gt-dialog-buttonzone-" + this.buttonLayout, id:this.domId + "_div"});
		...
		for (var A = 0; A < _.length; A++) {
			...
			if (_[A].breakline) {
				...
				if (_[A].html) {
					$ = GT.$e("span", {innerHTML:_[A].html});
				} else {
					$ = GT.$e("button", {id:this.domId + "_" + _[A].id, className:"gt-input-button", innerHTML:_[A].text});
...
GT.aL9 = function (C, $) {
	...
	var _ = C.columnList, D = ["<table class=\"gt-table\" style=\"margin-left:0px\" cellSpacing=\"0\"  cellPadding=\"0\" border=\"0\" >", "<col style=\"width:25px\" /><col style=\"width:105px\" />", "<thead>", GT.T_G.rowStart(C, 0), GT.T_G.cellStartHTML, B($), GT.T_G.cellEndHTML, GT.T_G.cellStartHTML, C.getMsg("COLUMNS_HEADER"), GT.T_G.cellEndHTML, GT.T_G.rowEndHTML, "</thead>", "<tbody>"];
	for (var E = 0; E < _.length; E++) {
		D.push([GT.T_G.rowStart(C, E), GT.T_G.cellStartHTML, A($, _[E]), GT.T_G.cellEndHTML, GT.T_G.cellStartHTML, _[E].header || _[E].title, GT.T_G.cellEndHTML, GT.T_G.rowEndHTML].join(""));
	}
	D.push("</tbody></table>");
	return D.join("\n");
};
GT.checkChecked = function (D) {
	D = GT.$grid(D);
	var F = D.chkAll, _ = GT.U.LG4("td", F), $ = GT.U.getCellIndex(_), H = D.getAllRows(), C = 0;
...
GT.lr2 = function (C, E) {
	var $ = C + "ColCheck", _ = E.gridId, G = _ + "_" + C + "ColDialog", D = GT.$grid(_), B = function () {
		var H = GT.$(G + "_div"), B = (GT.U.getTagName(H) == "TABLE") ? H : H.getElementsByTagName("table")[0], F = B.tBodies[0], A = F.getElementsByTagName("input"), C = GT.U.getCheckboxState(A, $), I = [];
		for (var J = 0; J < D.columnList.length; J++) {
		...
		GT.WidgetCache[G].close();
	}, F = new GT.Dialog({id:G, gridId:_, title:E.title, width:260, height:220, buttonLayout:"v", body:["<div id=\"" + G + "_div" + "\" onclick=\"GT.NP2.Qi7(event)\" class=\"gt-column-dialog\" >", "</div>"].join(""), buttons:[{text:D.getMsg("TEXT_OK"), onclick:B}, {text:D.getMsg("TEXT_CLOSE"), onclick:A}], afterShow:function () {
		var A = GT.$grid(this.gridId), _ = GT.aL9(this.gridId, {type:"checkbox", name:$, value:function ($) {
			return $.id;
...
GT.Vc8 = function (I) {
	var B = I.gridId, H = GT.$grid(B), K = B + "_filterDialog";
	...
	var A = function () {
		...
		var F = GT.$(K + "_div"), D = [], C = F.childNodes;
		for (var J = 0; J < C.length; J++) {
			if (GT.U.getTagName(C[J]) == "DIV" && C[J].className == "gt-filter-item") {
				var A = C[J].childNodes[1], B = C[J].childNodes[2], _ = C[J].childNodes[3].firstChild, G = GT.U.getValue(A), $ = H.columnMap[G];
				if ($ && $.fieldName) {
					D.push({columnId:G, fieldName:$.fieldName, logic:GT.U.getValue(B), value:GT.U.getValue(_)});
				}
			}
		}
		if (D.length > 0) {
		...
		GT.WidgetCache[K].close();
	}, $ = 430, E = 220, G = $ - (GT.isBoxModel ? 16 : 18), _ = E - (GT.isBoxModel ? 93 : 95), J = new GT.Dialog({id:K, gridId:B, title:I.title, width:$, height:E, buttonLayout:"h", body:["<div id=\"" + K + "_div\" class=\"gt-filter-dialog\" style=\"width:" + G + "px;height:" + _ + "px;\" onclick=\"GT.NP2.At4(event)\" >", "</div>"].join(""), buttons:[{html:GT.Ni3(H, K + "_column_select")}, {text:H.getMsg("TEXT_ADD_FILTER"), onclick:A}, {text:H.getMsg("TEXT_CLEAR_FILTER"), onclick:F}, {breakline:true}, {text:H.getMsg("TEXT_OK"), onclick:D}, {text:H.getMsg("TEXT_CLOSE"), onclick:C}], afterShow:function () {
		var E = GT.$grid(this.gridId), D = E.filterInfo || [];
		F();
		for (var I = 0; I < D.length; I++) {
			var G = D[I].columnId, A = E.getColumn(G), H = (A.header || A.title), C = GT.dX2(E, G, H), _ = C.childNodes[1], B = C.childNodes[2], $ = C.childNodes[3].firstChild;
			GT.U.setValue(_, G);
...
GT.dX2 = function (C, D, E) {
	C = GT.$grid(C);
	var B = GT.$e("div", {className:"gt-filter-item"}), A = "<input type=\"text\" readonly=\"readonly\" class=\"gt-input-text gt-filter-col-text\" value=\"" + E + "\" />";
	A += "<input type=\"hidden\"  value=\"" + D + "\" />";
	var $ = GT.Ip6(C, D), _ = "<button class=\"gt-input-button gt-filter-del\" >" + C.getMsg("TEXT_DEL") + "</button>" + "<button class=\"gt-input-button gt-filter-up\" >" + C.getMsg("TEXT_UP") + "</button>" + "<button class=\"gt-input-button gt-filter-down\" >" + C.getMsg("TEXT_DOWN") + "</button>";
	B.innerHTML = A + GT.T_D.Hg6 + $ + _;
	return B;
};
GT.NP2 = {currentElement:null, At4:function (A) {
	A = A || window.event;
	var E = GT.U.getEventTarget(A), _ = GT.U.LG4("table", null, A, 10);
	...
	A = A || window.event;
	var D = GT.U.getEventTarget(A), _ = GT.U.LG4("table", null, A, 10);
...
GT.EditorDefault = {gridId:null, left:0, top:0, render:GT.$empty, validator:null, isFocus:GT.$empty, onKeyPress:GT.$empty, errMsg:null, isActive:null, valueDom:null, locked:false, SO5:function ($) {
	if ($) {
		GT.$extend(this, $);
	}
	this.validator = this.validator || this.defaultValidator;
	if (GT.$type(this.validRule, "string")) {
		this.validRule = this.validRule.split(",");
	}
	if (this.required) {
		this.validRule = ["required"].concat(this.validRule);
	}
	this.dom = this.dom || GT.$e("div", {className:"gt-editor-container"});
	GT.U.addEvent(this.dom, "click", function ($) {
		GT.U.stopEvent($);
	});
	GT.U.addEvent(this.dom, "dblclick", function ($) {
		GT.U.stopEvent($);
	});
}, destroy:function () {
	this.container = null;
	...
	for (var B = 0; B < K.length; B++) {
		...
		var $ = GT.Validator.getValidator(G), A = true;
		if (GT.$type($, "function")) {
			A = $.apply($, J);
		}
		if (A !== true) {
...
GT.DialogEditor = GT.Editor.extend(GT.$extend({AO9:function () {
...
GT.Calendar = window.Calendar || {trigger:GT.$empty};
GT.$extend(GT.Editor, {Ol7:function ($, _) {
	if (GT.$type($, "function")) {
		$ = $(_);
	}
	...
	$ = new GT.Editor($);
	$.valueDom = GT.$e("input", {type:"text", value:$.defaultValue || "", className:"gt-editor-text"});
	$.dom.appendChild($.valueDom);
	return $;
}, textarea:function ($) {
	$ = new GT.Editor($);
	$.valueDom = GT.$e("textarea", {style:{width:$.width || "100px", height:$.height || "50px"}, value:$.defaultValue || "", className:"gt-editor-text"});
	$.dom.appendChild($.valueDom);
	...
	$ = new GT.Editor($);
	$.valueDom = GT.U.createSelect($.options, null, {className:"gt-editor-text"});
	$.dom.appendChild($.valueDom);
	$.getDisplayValue = function ($) {
	...
	_ = new GT.Editor(_);
	var $ = GT.$e("input", {type:"text", value:_.defaultValue || "", className:"gt-editor-text", style:{width:"78px", styleFloat:"left"}}), A = GT.$e("input", {type:"button", value:_.defaultValue || "", className:"gt-editor-date", styleFloat:"left"});
...
GT.Validator = {hasDepend:/^datetime|^date|^time|^minlength|^maxlength|^DT|^D|^T|^MINL|^MAXL/, hasArgument:/^equals|^lessthen|^EQ|^LT/, DATE_FORMAT:"yyyy-MM-dd", TIME_FORMAT:"HH:mm:ss", DATETIME_FORMAT:"yyyy-MM-dd HH:mm:ss", KeyMapping:{"R":"required", "DT":"datetime", "D":"date", "T":"time", "E":"email", "ID":"idcard", "N":"number", "int":"integer", "I":"integer", "F":"float", "M":"money", "RG":"range", "EQ":"equals", "LT":"lessthen", "GT":"greatethen", "U":"url", "ENC":"enchar", "CNC":"cnchar", "MINL":"minlength", "MAXL":"maxlength"}, RegExpLib:{"email":/\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*/, "number":/^\d+$/, "integer":/^[1-9]\d*|0$/, "float":/^([1-9]\d*\.\d+|0\.\d+|[1-9]\d*|0)$/, "money":/^([1-9]\d*\.\d{1,2}|0\.\d{1,2}|[1-9]\d*|0)$/, "telephone":/^[+]{0,1}(\d){1,3}[ ]?([-]?((\d)|[ ]){1,16})+$/, "enchar":/^[ \w]*$/, "cnchar":/^[\u4E00-\u9FA5\uF900-\uFA2D]*$/, "idcard":/^(\d{15}|\d{18}|\d{17}X?)$/i}, getValidator:function ($) {
	...
	for (var i = 1; i <= Yb8; i++) {
		...
		if (ostring && ostring.length > 2) {
			if (!ns) {
				msg = GT.U.replaceAll(msg, ostring[0], " " + ostring[2] + " ");
			} else {
				msg = GT.U.replaceAll(msg, ostring[0], " " + ns + " ");
			}
		}
	}
	return msg;
	...
	if (!GT.Validator.RegExpLib.telephone.exec($)) {
		return false;
	}
	return true;
}, "email":function ($) {
	...
	if (!_ || _.length < 15 || !GT.Validator.RegExpLib["idcard"].test(_)) {
		return false;
	}
	var $;
	if (_.length == 18) {
	...
	var C = /^\S+ \S+$/;
	if (!D || D.length < 1) {
		D = GT.Validator.DATETIME_FORMAT;
	} else {
		if (!C.test(D)) {
			return false;
		}
	...
	for (var A = 0; A < _.length; A++) {
		...
		var $ = _[A].split(" "), E = D.split(" "), B = GT.Validator.date($[0], E[0]) && GT.Validator.time($[1], E[1]);
		if (!B) {
			return false;
		}
	}
...
(function () {
	for (var $ in GT.Validator.KeyMapping) {
		GT.Validator[$] = GT.Validator[GT.Validator.KeyMapping[$]];
	}
})();
GT.Chart = GT.$class({SO5:function ($) {
...
GT.Chart.SWFMapping = {"column2D":"FCF_Column2D.swf", "pie3D":"FCF_Pie3D.swf"};
