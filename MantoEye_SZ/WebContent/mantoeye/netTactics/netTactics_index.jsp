...
function showMsg(id){
	showMMDialog('/modules/netTactics/net-tactics!edit.do?id='+id+'&readOnly=1','','600px','380px');
}
...
function showDetail(id,missionHour,missionTime){
	...
	obj.width = w;
	obj.height = h;
	window.showModalDialog("/mantoeye/netTactics/netTactics_detail.jsp",obj,"dialogWidth="+ w+ ";dialogHeight="+ h+ ";location=no;resizable=no;status=no;toolbar:no;scroll=yes;center=yes");
}
	

var gridConfig={
	id : "datagrid",
	loadURL :'/modules/netTactics/net-tactics!query.do?1=1',
	dataset : dsConfig ,
	columns : colsConfig ,
	width:780,
	height:500,
...
function addToolbar(addflag){
      ...
      var toolbar = new Toolbar({
        ...
        items : [{
          ...
          useable : bflags[0],
          handler : function(){
          		var v = showMMDialog('netTactics_input_1.jsp','','600px','380px');
          		reloadData(v);
          }
        },'-',{
          type : 'button',
          ...
          handler : function(){
          	...
          	if(count>1){
          		...
          		if(state!='未开始' && state!='停止'){//非未开始状态不能编辑
          			...
          			var v = showMMDialog('/modules/netTactics/net-tactics!edit.do?id='+id,'','600px','400px');
          			reloadData(v);
          		//}
          	}
          }
...
function deleteTactics(){
   ...
   var states = getColumnValueByName('datagrid','status');
	...
	if(flag){
		$.ajax({
			type : "post",
			url : "/modules/netTactics/net-tactics!deleteTactics.do",
			data : {
				ids:ids
			},
    ...
    if(getSelectedCount('datagrid')<1){
	  ...
	  var state = getColumnValueByName('datagrid','status');
		...
		if(flag){
			$.ajax({
				type : "post",
				url : "/modules/netTactics/net-tactics!stopTactics.do",
				data : {
					ids:ids
				},
				success : function(msg) {
