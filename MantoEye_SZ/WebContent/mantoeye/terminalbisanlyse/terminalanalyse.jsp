											...
											<table style="margin-left:5px;margin-top:10px;">
												<tr>
													<td colspan="2" height="40px">
														<span style="color:red;">温馨提示:</span> <span style="color:#0000FF;">终端业务分析,为了高效显示分析结果,分析显示列数最好控制<span style="color:red;">200</span>列以内,(例如:<span style="color:red;">40个终端类型*5个分析指标</span>,或 <span style="color:red;">100个终端类型*2个分析指标</span>,依此类推).</span>
													</td>
												</tr>
											</table>
	...
	$.ajax({
		...
		success : function(xml) {
			if(xml=="yes"){
				...
				obj.target = '/mantoeye/terminalbisanlyse/terminaldisplay.jsp?1=1';
	...
	function showDialog(){
		...
		if(dialog == null){
		  dialog = new SymbolDialog({
			...
			typeSearchName:'类型',
			modelSearchName:'业务',
			url:'terminalDialog_packBussnessInfoByTaskId.do',
			params:{
				taskId:taskId
			}
	      });
