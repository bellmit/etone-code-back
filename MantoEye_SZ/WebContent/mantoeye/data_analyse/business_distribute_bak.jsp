					...
					<table cellspacing="0" cellpadding="0" border="0" bgcolor="#ffffff"
								...
								<table cellspacing="0" cellpadding="0" border="0" width="100%">
									...
									<tr valign="top">
										<td width="100%" class="condition_down">
											<table id="query_condition" cellspacing="0px" style="margin-left:1px"
													...
													<td width="300px">
														<div id="mainbtn" style="width:300px">
															<ul>
																...
																<li>
																	<a href="javascript:parent.openTab('/mantoeye/data_trend/business_distribute_trend.jsp','FLUSH_BUSI_DATE','业务时间走势');" 
																		style="width:90px;"  title="时间走势"><span><img onclick="parent.openTab('/mantoeye/data_trend/business_distribute_trend.jsp','FLUSH_BUSI_DATE','业务时间走势');" 
											...
											<table style="margin-left:-1px">
												<tr valign="middle">
													<td
														...
														<input type="radio" value="businesstype" checked name="businessType" id="d_businessType" onclick="selectBusiness(this);"
															style="width: 20px; border: 0px; background-color: transparent;" />
														业务类型
													</td>
													<td
														style="width: 72px; cursor: default; padding: 0px 1px 0px 10px; color: #ffffff; font-size: 11px;">
														<input type="radio" value="business" name="businessType" onclick="selectBusiness(this);"
															style="width: 20px; border: none; background-color: transparent;" disabled id="d_businessss"/>
														业务
													</td>
													...
													<td height="22px;">
														...
														<input id="businessIds" type="hidden" value="" name="businessIds" />
													</td>
												</tr>
											</table>
										</td>
								...
								<table cellspacing="0" cellpadding="0" border="0" width="100%"
									...
									<tr valign="top">
										...
										<td width="100%" height="24px">
											...
											<%--柱状图 --%>
											<input type="hidden" value="" id="chartxmldata9" />								
										</td>
										<td width="4px">
						...
						<tr>
							<td height="230px" width="100%"
								...
								<div id="imgareas1" style="width: 100%; height: 100%;border: 2px solid #008BD1;display:block;">
								...
								<div id="imgareas2" style="width: 100%; height: 100%;border: 2px solid #008BD1;display:none;">
...
function renderInit(value ,record,columnObj,grid,colNo,rowNo){
		...
		return '<table><tr><td><img class="handImg1" style="cursor: hand" src="/skin/Default/images/MantoEye/icon/map.png" alt="查看空间分布" onclick="openSapceDistribute(1,\''+businessOne+'\',-1);" /></td>'
					+'</td><td></div><img class="handImg2" style="cursor: hand" src="/skin/Default/images/MantoEye/icon/business.png" alt="查看具体业务分布情况" onclick="openSapceDistribute(2,\''+businessTypeOne+'\','+totalImsis+');" /></td>'
					+'</tr></table>';
}
var colsConfig = [
...
var gridConfig={
	...
	loadURL :'/businessDistribute_query.do?1=1&total_time_level='+parent.total_obj.total_time_level+'&total_time_search='+parent.total_obj.total_time_search,
...
function buildChartData(totalRecords){
	...
	if(!chartinit){
		...
		swfobject.embedSWF("/flash/open-flash-chart-mw.swf?id=flush", "imgarea1", "33%", "100%", "9.0.0","expressInstall.swf",'',{"wmode":"opaque"});
		swfobject.embedSWF("/flash/open-flash-chart-mw.swf?id=imsis", "imgarea2", "66%", "100%", "9.0.0","expressInstall.swf",'',{"wmode":"opaque"});
...
function buildChartData(totalRecords){
		...
		for(var i = 0 ; i<totalRecords;i++){	
			var record = datagrid.getRecord(i);	 	
		 	...
		 	if(level==1){
		 		...
		 		if(i<10){ 
		 			datas1[i] = {"label":record.businessType,"value":StringToFloat(record.totalFlushKB)};
		 			datas2[i] = {"label":record.businessType,"value":StringToFloat(record.intImsis)};
					datas3[i] = {"label":record.businessType,"value":StringToFloat(record.visit)};
				}else{
					otherflush = otherflush + StringToFloat(record.totalFlushKB);
		 		...
		 		if(i<10){ 
					...
					otherflush = otherflush + StringToFloat(record.totalFlushGB);
					otherimsis = otherimsis + StringToFloat(record.intImsis);
					othercount = othercount + StringToFloat(record.visit);
				}	
		 	}else{
	...
	if(!chartinit){
		...
		document.getElementById("chartxmldata2").value=str2;
		document.getElementById("chartxmldata3").value=str3;
		swfobject.embedSWF("/flash/open-flash-chart-mw.swf?id=flush", "imgarea1", "49%", "100%", "9.0.0","expressInstall.swf",'',{"wmode":"opaque"});
		swfobject.embedSWF("/flash/open-flash-chart-mw.swf?id=imsis", "imgarea2", "49%", "100%", "9.0.0","expressInstall.swf",'',{"wmode":"opaque"});
		swfobject.embedSWF("/flash/open-flash-chart-mw.swf?id=count", "imgarea3", "0%", "0%", "9.0.0","expressInstall.swf",'',{"wmode":"opaque"});
	}else{
	  	var tmp1 = findSWF("imgarea1");	  	
  		tmp1.load(str1);
...
function callback(label){
	...
	if(flag==0){
		...
		var value = window.showModalDialog("/mantoeye/data_analyse/business_in_space.jsp",obj,"dialogWidth="+w+";dialogHeight="+h+";location=yes;resizable=no;status=no;toolbar:no;scroll=no;center=yes");
	}
}
///////////////////////////////////////////
...
function query() {
	...
	datagrid.loadURL ='/businessDistribute_query.do?1=1&total_time_level='+parent.total_obj.total_time_level+'&total_time_search='+parent.total_obj.total_time_search;
	/**
