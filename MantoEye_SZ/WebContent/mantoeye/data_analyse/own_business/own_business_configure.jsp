...
function renderInit(value ,record,columnObj,grid,colNo,rowNo){
	...
	var thtml = "<table><tr>";
	thtml = thtml +'<td><img style="cursor: hand" src="/skin/Default/images/MantoEye/icon/info.png" alt="选择竞争对手分析" onclick="openDistributeBusiness(1,\''+businessName+'\',\'' +id+'\');" /></td>';
...
var colsConfig = [
		...
		{ id : 'businessName' , header : "业务名称" ,exportnumber:true},
		{ id : 'vcNote' , header : "备注" ,exportnumber:true},
...
var gridConfig={
	id : "datagrid",
	loadURL :'/ownBusinessConfigure_query.do?1=1',
	exportURL :'/ownBusinessConfigure_export.do?1=1',
	dataset : dsConfig ,
	columns : colsConfig ,
	width:780,
	height:500,
...
//列表链接操作
function openDistributeBusiness(flag,businessName,id){
	var obj =new Object();
	obj.id = id ;
	obj.businessName = businessName;
	showMMDialog("/competitorBusinessConfigure_competitorBusiness.do?id="+id,obj,'680px','500px');
	//showMMDialog("/mantoeye/data_analyse/own_business/competitor_business_configure.jsp",obj,'680px','500px');
...
function query() {
	...
	//查询条件
	businessName = $('#businessName').attr("value");
	//传递给后台参数
	var param={
...
$(document).ready(function(){
      var toolbar = new Toolbar({
        ...
        items : [{
          ...
          useable : 'T',
          handler : function(){
          		var mm= showMMDialog('/mantoeye/data_analyse/own_business/own_business_configure_index.jsp','重点业务','680px','500px');
...
function deleteBusiness(){
	...
	if(flag){
		$.ajax({
			type : "post",
			url : "ownBusinessConfigure_deleteBusiness.do",
			data : {
				ids:ids
			},
