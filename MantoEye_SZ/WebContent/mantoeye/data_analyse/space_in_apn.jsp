						...
						<tr valign="top">
							<td>
								...
								<div id="active_count_image" style="width: 0; height: 0;margin-left:-13px;overflow:hidden;">
									<iframe name="frm2"
										src="/flash/MantoEye_Main.html?mapflag=bsc&dataflag=count&dataname=激活次数"
...
if(dataTypeValue=='2'){
	...
	//TD没有SGSN
	$("#spaceLevel_search").empty();
	var inhtml = '<option value="1">BSC</option><option value="3">街道办</option><option value="4">营销片区</option><option value="5">分公司</option>';
	$(inhtml).appendTo("#spaceLevel_search");
}
//alert(searchDateStart);
var spaceType=1;//区域维度(BSC:1 SGSN:2 街道办:3 营销片区:4 分公司:5	)
var datachange = false;//第N次查询初始化数据标识
$('#distribute_data_div').html(apnName+'分布数据');
$('#distribute_chart_div').html(apnName+'分布图形');
...
var colsConfig = [
		...
		{ id : 'businessName'      , header : "空间名称" ,sortable:false  ,width:150    },
		...
		{ id : 'upFlushKB'    , header : "上行流量(KB)" ,sortable:false ,width:150 ,headAlign:'right' ,align:'right' ,exportnumber:true ,renderer:renderFormatDataInit2  },
		{ id : 'downFlushKB'     , header : "下行流量(KB)" ,sortable:false ,width:150 ,headAlign:'right' ,align:'right' ,exportnumber:true ,renderer:renderFormatDataInit2   },
		{ id : 'totalFlushKB'  , header : "总流量(KB)" ,sortable:false ,width:150 ,headAlign:'right' ,align:'right',exportnumber:true,renderer:renderFormatDataInit2 },
		{ id : 'upFlushGB'    , header : "上行流量(GB)" ,sortable:false ,width:150 ,headAlign:'right' ,align:'right' ,exportnumber:true ,renderer:renderFormatDataInit2  },
		{ id : 'downFlushGB'     , header : "下行流量(GB)" ,sortable:false ,width:150 ,headAlign:'right' ,align:'right' ,exportnumber:true ,renderer:renderFormatDataInit2   },
		{ id : 'totalFlushGB'  , header : "总流量(GB)" ,sortable:false ,width:150 ,headAlign:'right' ,align:'right',exportnumber:true,renderer:renderFormatDataInit2 },
		{ id : 'intImsis' , header : "用户数"  ,sortable:false ,width:130 ,headAlign:'right' ,align:'right' ,exportnumber:true  },
		...
		{ id : 'visit' , header : "激活次数" ,sortable:false  ,width:130 ,headAlign:'right' ,align:'right' ,exportnumber:true  }
];

var gridConfig={
	id : "datagrid",
	loadURL :'/spaceinapn_querySpaceInApn.do?apnName='+(encodeURIComponent(obj.apnName))+'&dataType='+dataTypeValue+'&searchDateStart='+searchDateStart+'&timeLevel_search='+obj.timeLevel_search+'&date='+date,
	exportURL :'/spaceinapn_export.do?1=1' ,
	dataset : dsConfig ,
	columns : colsConfig ,
	width:780,
	height:500,
	resizable : false,
	beforeLoad:checkPopLogon,
...
function loadComplate(){
	var  obj1 = new Object ();//使用对象传参,以防以后修改
	obj1.datagrid = datagrid;
	obj1.hideColumn = 7 ;//隐藏的列数目
	obj1.isCheckbox = false;//是否可选择
	obj1.otherHeight = 0 ; //必须定义,因为有些页面不同，所以此处指定预留高度
...
function getAjaxChartXmlDataSpace(){
	$.ajax({
		type : "post",
		url : "spaceinapn_getAjaxChartXmlSpaceData.do?1=1",
		data : {
...
function showAreaType(){
	var st=$("#spaceLevel_search option:selected").val(); 
	switch(st)
	   {
	   case '1':
	   	 chartFlag = 'bsc';
