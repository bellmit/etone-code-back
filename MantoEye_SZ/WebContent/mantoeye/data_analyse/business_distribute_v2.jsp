					...
					<tr valign="top">
						...
						<table id="query_condition" cellspacing="0px"
							...
							<tr valign="middle">
								<td style="width: 72px">
								<input type="radio"
									name="dataType" id="d_dataType1" checked
									style="width: 20px; border: 0px; background-color: transparent;" 
									onclick='showDataType(1)' />
								GPRS</td>
								<td style="width: 72px"><input type="radio"
									name="dataType" id="d_dataType2"
								...
								<td class="condition_value"><input type="text"
									class="Wdate" style="height: 17px;" onclick="preStartDate();" onchange="dateChange()"
			...
			<tr>
				...
				<table cellspacing="0" cellpadding="0" border="0" width="100%"
					...
					<tr valign="top">
						...
						<div class="middletitle">业务分布图形</div>
...
var dataTypeValue = parent.total_obj.dataType_search==null?'1':parent.total_obj.dataType_search; //数据类型标识 1GPRS业务、2TD业务
setDataTypeCheck(dataTypeValue);	//设置选中
...
function renderInit(value ,record,columnObj,grid,colNo,rowNo){
		...
		return '<img style="cursor: hand" src="/skin/Default/images/MantoEye/icon/business.png" alt="查看具体业务分布情况" onclick="openSapceDistribute(2,\''+businessTypeOne+'\','+totalImsis+','+nmGroupId+');" />'
...
var colsConfig = [
		...
		{ id : 'nmGroupId'      , header : "nmGroupId" , sortable:false ,hidden:true },
		{ id : 'business'      , header : "业务" , sortable:false  },
		{ id : 'businessType'      , header : "业务类型"  ,sortable:false  },
...
var gridConfig={
	...
	loadURL :'/businessDistributeV2_query.do?1=1&total_time_level='+parent.total_obj.total_time_level+'&total_time_search='+parent.total_obj.total_time_search+'&dataType='+dataTypeValue,
...
function callback(label){
	...
	if(flag==0){
		...
		obj.totalImsis = getTotalImsiByBusi(label);
	 	var value = window.showModalDialog("/mantoeye/data_analyse/business_in_businesstype.jsp",obj,"dialogWidth="+w+";dialogHeight="+h+";location=no;resizable=no;status=no;toolbar:no;scroll=yes;center=yes");
	}else{
		obj.dataTypeValue = dataTypeValue;
		obj.searchDateStart = searchDateStart ;
		obj.timeLevel_search = timeLevel_search ;
...
function query() {
	...
	datagrid.loadURL ='/businessDistributeV2_query.do?1=1&total_time_level='+parent.total_obj.total_time_level+'&total_time_search='+parent.total_obj.total_time_search;
	/**
	...
	if(num==2){
		...
		if(compareDate()){
			...
			success : function(xml) {
				if(xml=='true'){
					var value = window.showModalDialog("/mantoeye/data_analyse/business_in_businesstype_v2.jsp",obj,"dialogWidth="+w+";dialogHeight="+h+";location=no;resizable=no;status=no;toolbar:no;scroll=yes;center=yes");
				}
				if(xml=='false'){
					var value = window.showModalDialog("/mantoeye/data_analyse/business_in_businesstype_v3.jsp",obj,"dialogWidth="+w+";dialogHeight="+h+";location=no;resizable=no;status=no;toolbar:no;scroll=yes;center=yes");
				}
			},
			error : function() {
				//alert('查询数据出错!');
		...
		if(compareDate()){
			var value = window.showModalDialog("/mantoeye/data_analyse/business_in_space.jsp",obj,"dialogWidth="+w+";dialogHeight="+h+";location=no;resizable=no;status=no;toolbar:no;scroll=yes;center=yes");
		}else{
			var value = window.showModalDialog("/mantoeye/data_analyse/business_in_space_v2.jsp",obj,"dialogWidth="+w+";dialogHeight="+h+";location=no;resizable=no;status=no;toolbar:no;scroll=yes;center=yes");
		}
