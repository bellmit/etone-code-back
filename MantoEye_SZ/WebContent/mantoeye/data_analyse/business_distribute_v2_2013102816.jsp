										...
										<table id="query_condition" cellspacing="0px"
											...
											<tr valign="middle">
												...
												<td class="condition_value"><input type="text"
													...
													onclick="preStartDate();" onchange="dateChange()"
												...
												<td width="300px">
													<div id="mainbtn" style="width: 300px">
														<ul>
															...
															<li><a
																href="javascript:parent.openTab('/mantoeye/data_trend/business_distribute_trend_v2.jsp','FLUSH_BUSI_DATE_v2','业务时间走势');"
																style="width: 90px;" title="时间走势"><span><img
																		onclick="parent.openTab('/mantoeye/data_trend/business_distribute_trend_v2.jsp','FLUSH_BUSI_DATE_v2','业务时间走势');"
																		src="/skin/Default/images/MantoEye/btn/trend.png"
																		alt="" />时间走势</span> </a></li>
														</ul>
													</div>
												</td>
										...
										<table style="margin-left: -1px">
											<tr valign="middle">
												...
												<td
													...
													disabled id="d_businessss" /> 业务</td>
							...
							<table cellspacing="0" cellpadding="0" border="0" width="100%"
								...
								<tr valign="top">
									...
									<td width="100%" height="24px">
										<div class="middletitle">业务分布图形</div> <%--饼图 --%> <input
										type="hidden" value="" id="chartxmldata1" /> <input
						...
						<td>
							<table cellspacing="0" cellpadding="0" border="0" width="100%"
								...
								<tr valign="top">
									...
									<td width="100%" height="24px">
										<div class="middletitle">业务分布列表</div>
									</td>
									<td width="4px">
										<div class="righttitle"></div>
									</td>
...
<script type="text/javascript">
	...
	function compareDate() {
		...
		var searchDate = searchDateStart.split('-');
	...
	var dataTypeValue = parent.total_obj.dataType_search == null ? 1
			: parent.total_obj.dataType_search;
	...
	var month = ((searchDateStart.getMonth() + 1) > 9 ? (searchDateStart
			.getMonth() + 1) : ("0" + (searchDateStart.getMonth() + 1)));
	var day = (searchDateStart.getDate() > 9 ? searchDateStart.getDate()
			: ("0" + searchDateStart.getDate()));
	var defaulettime = searchDateStart.getYear() + "-" + month + "-" + day;

	var timeLevel_search = parent.total_obj.total_time_level == null ? '2'
			: parent.total_obj.total_time_level;//查询时间粒度
	searchDateStart = parent.total_obj.total_time_search == null ? defaulettime
			: parent.total_obj.total_time_search;

	var business;//业务
	var businessType = "businesstype";//业务类型
	...
	function renderInit(value, record, columnObj, grid, colNo, rowNo) {
		...
		var nmGroupId = record.nmGroupId;
		var totalImsis = record.intImsis;
		return '<img style="cursor: hand" src="/skin/Default/images/MantoEye/icon/business.png" alt="查看具体业务分布情况" onclick="openSapceDistribute(2,\''
				+ businessTypeOne
				+ '\','
				+ totalImsis
				+ ','
				+ nmGroupId
				+ ');" />'
		//'<table><tr><td><img class="handImg1" style="cursor: hand" src="/skin/Default/images/MantoEye/icon/map.png" alt="查看空间分布" onclick="openSapceDistribute(1,\''+businessOne+'\',-1);" /></td>'
		//'<img style="cursor: hand" src="/skin/Default/images/MantoEye/icon/business.png" alt="查看具体业务分布情况" onclick="openSapceDistribute(2,\''+businessTypeOne+'\','+totalImsis+','+nmGroupId+');" />'
	}
	var colsConfig = [ {
		id : 'fullDate',
		header : "时间",
...

	{
		...
		headAlign : 'right',
		align : 'right',
		renderer : renderFormatDataInit4
	}, {
		id : 'intImsis',
		header : "用户数",
		headAlign : 'right',
	...
	var gridConfig = {
		...
		loadURL : '/businessDistributeV2_query.do?1=1&total_time_level='
				+ parent.total_obj.total_time_level + '&total_time_search='
				+ parent.total_obj.total_time_search + '&dataType='
				+ dataTypeValue,
		exportURL : '/businessDistributeV2_export.do?1=1',
	...
	function buildBarChartData(totalRecords) {
		...
		if (!barchartinit) {
			...
			swfobject.embedSWF("/flash/open-flash-chart-mw.swf?id=business",
					"imgarea9", "100%", "100%", "9.0.0", "expressInstall.swf",
					'', {
						"wmode" : "opaque"
					});
		} else {
	...
	function buildChartJsonStr(totalRecords, flag) {
		...
		if (typeof (datagrid.getRecord(0)) != "undefined") {
			...
			if (csortorder != "desc" && csortorder != "asc") {
				if (flag == "flush") {
					if (datagrid.getRecord(i).totalFlushMB > datagrid
							.getRecord(0).totalFlushMB) {
						csortorder = "asc";
					} else {
						csortorder = "desc";
					}
			...
			if (csortorder == "desc") {
				...
				for ( var i = 0; i < totalRecords; i++) {
					...
					datas1[i] = addCallFunction(record.business,
							StringToFloat(record.totalFlushMB));
					datas2[i] = addCallFunction(record.business,
							StringToFloat(record.intImsis));
				...
				for ( var i = (totalRecords - 1); i >= 0; i--) {
					...
					//如果柱状图需要添加点击，必须以如下的方法设置个点的值	
					datas1[totalbar] = addCallFunction(record.business,
							StringToFloat(record.totalFlushMB));
					datas2[totalbar] = addCallFunction(record.business,
							StringToFloat(record.intImsis));
					totalbar++;
				}
			}
	...
	function buildChartData(totalRecords) {
		...
		if (typeof (datagrid.getRecord(0)) != "undefined") {
			for ( var i = 0; i < totalRecords; i++) {
				...
				if (text1 == '') {
					chartTime = record.fullDate;
					text1 = chartTime + ' 业务类型流量占比图';
					text2 = chartTime + ' 业务类型用户数占比图';
					text3 = chartTime + ' 业务类型激活次数占比图';
				}

				if (level == 1) {
					label1 = label1 + "@@KB";
					datas1[i] = {
						"label" : record.businessType,
						"value" : StringToFloat(record.totalFlushKB)
					};
				} else if (level == 3) {
					label1 = label1 + "@@GB";
					datas1[i] = {
						"label" : record.businessType,
						"value" : StringToFloat(record.totalFlushGB)
					};
				} else {
					label1 = label1 + "@@MB";
					datas1[i] = {
						"label" : record.businessType,
						"value" : StringToFloat(record.totalFlushMB)
					};
				}
				...
				datas3[i] = {
					"label" : record.businessType,
					"value" : StringToFloat(record.visit)
				};
			}
			datas2 = buildHBarChartTip(datas2, showxlabels, label2, showxdatas,
					label22);

			var jsons1 = buildPieChartData(text1, label1, datas1);
			var str1 = JSON.stringify(jsons1);
			//var jsons2 = buildPieChartData(text2,label2,datas2);	
			var jsons2 = buildPercentChartData(text2, [ label2 ], xlabels2,
					[ datas2 ]);
			...
			if (!chartinit) {
				...
				swfobject.embedSWF("/flash/open-flash-chart-mw.swf?id=flush",
						"imgarea1", "40%", "100%", "9.0.0",
						"expressInstall.swf", '', {
							"wmode" : "opaque"
						});
				swfobject.embedSWF("/flash/open-flash-chart-mw.swf?id=imsis",
						"imgarea2", "59%", "100%", "9.0.0",
						"expressInstall.swf", '', {
							"wmode" : "opaque"
						});
				swfobject.embedSWF("/flash/open-flash-chart-mw.swf?id=count",
						"imgarea3", "0%", "0%", "9.0.0", "expressInstall.swf",
						'', {
							"wmode" : "opaque"
						});
			} else {
			...
			swfobject.embedSWF("/flash/open-flash-chart.swf?id=imsis",
					"imgarea2", "0%", "0%", "9.0.0", "expressInstall.swf", '',
					{
						"wmode" : "opaque"
					});
			swfobject.embedSWF("/flash/open-flash-chart.swf?id=count",
					"imgarea3", "0%", "0%", "9.0.0", "expressInstall.swf", '',
					{
						"wmode" : "opaque"
					});
	...
	function callback(label) {
		...
		if (flag == 0) {
			...
			var value = window
					.showModalDialog(
							"/mantoeye/data_analyse/business_in_businesstype.jsp",
							obj,
							"dialogWidth="
									+ w
									+ ";dialogHeight="
			...
			var value = window
					.showModalDialog(
							"/mantoeye/data_analyse/business_in_space.jsp",
							obj,
							"dialogWidth="
									+ w
									+ ";dialogHeight="
									+ h
									+ ";location=yes;resizable=no;status=no;toolbar:no;scroll=yes;center=yes");
		}
	}
	///////////////////////////////////////////
	...
	function query() {
		...
		datagrid.loadURL = '/businessDistributeV2_query.do?1=1&total_time_level='
				+ parent.total_obj.total_time_level
				+ '&total_time_search='
				+ parent.total_obj.total_time_search;
		/**
		...
		var toolbar = new Toolbar({
			...
			items : [ {
				...
				handler : function() {
					...
					fileObj.fileName = '业务分布数据';//('+startTime_search+'至'+endTime_search+')';//文件名称
	...
	function openSapceDistribute(num, bis, imms, nmGroupId) {
		if (nmGroupId == null) {
			nmGroupId = 0;
		}
		var obj = new Object();
		if (num == 2) {
			...
			if (compareDate()) {
				...
				$
						.ajax({
							type : "post",
							url : "businessDistributeV2_isExistLeaf.do?1=1",
							data : {
								nmGroupId : nmGroupId
							},
							success : function(xml) {
								if (xml == 'true') {
									var value = window
											.showModalDialog(
													"/mantoeye/data_analyse/business_in_businesstype_v2.jsp",
													obj,
													"dialogWidth="
															+ w
															+ ";dialogHeight="
								...
								if (xml == 'false') {
									var value = window
											.showModalDialog(
													"/mantoeye/data_analyse/business_in_businesstype_v3.jsp",
													obj,
													"dialogWidth="
															+ w
															+ ";dialogHeight="
			...
			if (compareDate()) {
				...
				var value = window
						.showModalDialog(
								"/mantoeye/data_analyse/business_in_space_v2.jsp",
								obj,
								"dialogWidth="
										+ w
										+ ";dialogHeight="
	...
	function showDialog() {
		...
		if (dialog == null) {
			...
			dialog.url = url;
			dialog.removeData();//先设置属性hasInit:true才有效
			dialog.init();//初始化页面DOM对象
			dialog.loadData();//加载后台数据
