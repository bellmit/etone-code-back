...
var dataType_search = parent.total_obj.dataType_search==null?'1':parent.total_obj.dataType_search; //页面显示数据类型
...
setDataTypeCheck(dataType_search);
...
function renderInit(value ,record,columnObj,grid,colNo,rowNo){
	...
	return '<table><tr><td><img class="handImg1" style="cursor: hand" src="/skin/Default/images/MantoEye/icon/business.png" alt="查看业务分布" onclick="openDistributeBusiness(1,\''+businessName+'\');" /></td>'
					+'</td><td></div><img class="handImg2" style="cursor: hand" src="/skin/Default/images/MantoEye/icon/business.png" alt="查看内容分布" onclick="openDistributeBusiness(2,\''+businessName+'\');" /></td>'
					+'</tr></table>';
	//'<img style="cursor: hand" src="/skin/Default/images/MantoEye/icon/business.gif" alt="查看业务分布" onclick="openDistributeBusiness(\''+businessName+'\');" />'
}
var colsConfig = [
...
var gridConfig={
	...
	loadURL :'/spaceDistribute_query.do?total_time_level='+parent.total_obj.total_time_level+'&total_time_search='+parent.total_obj.total_time_search+'&dataType_search='+dataType_search,
...
function setDataTypeCheck(dataType_search){
	if(dataType_search == 1){
		$('#dataType_search_td_id input:first').attr("checked","true");//设置radio默认值
	}else if(dataType_search == 2){
		$('#dataType_search_td_id input:last').attr("checked","true");//设置radio默认值
	}
}
...
function openDistributeBusiness(flag,businessName){
	...
	if(flag==1){
		if(!compareDate()){
			window.showModalDialog("/mantoeye/data_analyse/space_distribute_business_v2.jsp",obj,"dialogWidth="+w+";dialogHeight="+h+";location=no;resizable=no;status=no;toolbar:no;scroll=yes;center=yes");
		}else{
		...
		window.showModalDialog("/mantoeye/data_analyse/space_distribute_context_v2.jsp",obj,"dialogWidth="+w+";dialogHeight="+h+";location=no;resizable=no;status=no;toolbar:no;scroll=yes;center=yes");
	}
