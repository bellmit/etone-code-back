			...
			<tr>
				<td>
					<table cellspacing="0" cellpadding="0" border="0" bgcolor="#ffffff"
						...
						<tr>
							<td>
						<div id="toolbar" style="height: 25px" style="display:none"></div>
								<div class="gt-panel"
...
var dsConfig= {
	//data : data1 ,
	uniqueField : 'nmTaskFileInfoId' ,
	fields :[
		{name : 'nmTaskFileInfoId'        },
		{name : 'serverIP'        },
		{name : 'serverName'        },
		{name : 'dtETime'      },
		{name : 'nmFileSize'      },
...
var colsConfig = [
		{ id : 'vcFilePath'      , header : "文件路径"    ,width:180 },
		{ id : 'nmFileSize'      , header : "文件大小(KB)"   ,width:80  },
		{ id : 'vcFileFormat'      , header : "文件类型"   ,width:80   },
		{ id : 'dtETime'      , header : "生成时间"   ,width:120 },
		{ id : 'detail'   , header : "下载" , sortable:false ,width:55 , 
			renderer:renderManager
		}
];
function renderManager(value ,record,columnObj,grid,colNo,rowNo){
	var fileid = record.nmTaskFileInfoId.toString();
	var path = record.vcFilePath.toString();
	return '<table><tr><td><img style="cursor: hand"  src="/skin/Default/images/MantoEye/icon/download.gif" alt="下载文件" onclick="downloadFile(\''+fileid+'\',\''+path+'\');" /></td>'
			+'</tr></table>';
}
function downloadFile(fileid,path){	
		//document.location.href="/fileInfo_downloadForRomote.do?fileid="+fileid+"&isDec=false";
	//document.location.href="/protocolDistribute_query.do?1=1&";
	var tn = encodeURIComponent(taskName);
	 openUrl("/fileInfo_downloadForRomote.do?fileid="+fileid+"&isDec=false&taskType=5&taskName="+tn,"_blank");
}
var gridConfig={
	id : "datagrid",
	/* loadURL 支持函数, 该函数返回值是response对象 */
	loadURL :'/fileInfo_query.do?taskid='+taskid,
	exportURL :'/fileInfo_export.do?taskid='+taskid ,
	dataset : dsConfig ,
	columns : colsConfig ,
	resizable : false,
	showGridMenu : false,
