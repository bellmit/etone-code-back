...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<head>
		<title>运营商区域分布</title><meta http-equiv="X-UA-Compatible" content="IE=EmulateIE7" />
												...
												<tr>
													...
													<td style="width:300px" colspan="4">														
														<select name="ldcId" id="ldcId"
																style="height: 21px; width: 125px;" onchange="">
																<option value="-1">
																	--全部--
																</option>
														</select>														
...
var gridConfig={
	...
	loadURL :'/blackberryyysarea_query.do?1=1',
	exportURL :'/blackberryyysarea_export.do?1=1' ,
...
function getAjaxChartXmlData(){
	$.ajax({
		type : "post",
		url : "blackberryyysarea_getAjaxChartXmlData.do",
		data : {
			dataTypeValue:1,
        ...
        items : [{
          ...
          handler : function(){
           //参数
			...
			var ename = yysName.replace(/\(\S*\)/g,'');
			fileObj.fileName=ename+'黑莓空间分布数据';//('+startTime_search+'至'+endTime_search+')';//文件名称
	...
	$.ajax({
		...
		success : function(data) {
			...
			for(var i=0;i<jn.length;i++){
				if(jn[i].name=='中国移动'){
					inhtml = inhtml + '<option value ="'+jn[i].name+'" selected >'+jn[i].name+'</option>';
				}else{
					inhtml = inhtml + '<option value ="'+jn[i].name+'"  >'+jn[i].name+'</option>';
				}
				ldcs[i] = {name:jn[i].name,id:jn[i].name};
			}
			 $(inhtml).appendTo("#ldcId");
		},
		error : function() {
			alert('获取运营商数据失败!');
		}
