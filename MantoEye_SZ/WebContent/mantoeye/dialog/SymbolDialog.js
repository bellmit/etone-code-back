 ...
 * 
 * */
document.write("<script type='text/javascript'	src='/tools/autocomplete/lib/jquery.bgiframe.min.js'></script>");
document.write("<script type='text/javascript'	src='/tools/autocomplete/lib/thickbox-compressed.js'></script>");
document.write("<script type='text/javascript'	src='/tools/autocomplete/jquery.autocomplete.js'></script>");
document.write("<link rel=\"stylesheet\" type=\"text/css\"	href=\"/tools/autocomplete/jquery.autocomplete.css\" />");
...
SymbolDialog = function (config) {
	...
	this.addSearchId=config.id + "img_add_search";
	this.delAllId = config.id + "img_delete_all";
	this.delSearchId = config.id + "img_delete_search";
...
SymbolDialog.prototype = {init:function () {
	...
	this.searchClickAllForAutoComplete();//新增全业务范围查询，增加自动补全功能
	...
	var imgAllContext ='';
		...
		if(typeof this.imgtypeall=='undefined'||this.imgtypeall==true){
		 	imgAllContext=imgAllContext+"<img id=\"" + this.addAllTypeId + "\" src=\"/skin/Default/images/icon/16/atype.png\" alt=\"添加所有搜索结果\"></img> " + "<br /><br />" + "<img id=\"" + this.delAllTypeId + "\" src=\"/skin/Default/images/icon/16/rtype.png\" alt=\"删除所选结果\"></img><br/><br/><br/>";
		}
	...
	$(rightListSearchContext).appendTo($bodyTR);
...
/*}, searchClick:function (divObj) {//查询事件
...
}, searchClickAllForAutoComplete:function (divObj) {//新增全业务范围查询
	var o = this;
	var v = o.mId;

	/*
	 $("#" + o.mId).blur(function () {
		// alert('快速查詢，自動補全。。。BLUR');
		 checkBlur(v);
	});
	*/
	 
	$("#" + o.mId).click(function () {
		//alert('快速查詢，自動補全。。。CLICK');
		checkFocus(v);
	});
	 
	$("#" + o.mId).mousedown(function () {
		checkFocus(v);
	});
	...
	*/
    ...
    var o = this;
    var vvv = $('#'+o.rightIdForSearch).val();
	var valueTempArr = vvv.split(',');
		...
		if (search_value == null || search_value == '' || search_value == 'null') {//非查询事件改变
			...
			if (mnas[i].toLowerCase().indexOf(search_value) != -1) {
				$("#" + o.leftId).append("<option value='" + mids[i].trim() + "'>" + mnas[i].trim() + "</option>");
 				modelIds += mids[i].trim() +',';
				modelNames += mnas[i].trim()+',';
				total = total + 1;
			}
		}
	}
	...
	for(var i = 0 ; i < vvv.length ; i ++){
		var vvss = vvv[i];
		var vvvvv = vvss.split(':');
		o.ldcs[i] = {name:vvvvv[2],id:vvvvv[2]};
	}

	$("#" + o.rightIdForSearch).attr('value',valueTemp);
	resetautocomplete(o.ldcs,o.mId,o.leftId);
},resetautocomplete:function(data,input,render){
...
},checkFocus:function (inputId) {//显示复选框
	$("#"+inputId).attr("value","");
