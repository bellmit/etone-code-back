...
//列表链接操作
function openDistributeBusiness(flag, vcTerminalBrand, vcName,nmAreaId) {
	var obj = new Object();
	obj.areaType = areaType;
	obj.timeLevel_search = timeLevel_search;
	obj.dataType_search = dataType_search;
	obj.time_search = time_search;
	obj.vcTerminalBrand = vcTerminalBrand;
	obj.vcName = vcName;
	obj.nmAreaId = nmAreaId;
	if (flag == 1) {
		window.showModalDialog("/mantoeye/terminal/terminal_brand_business.jsp",obj,"dialogWidth="+ w+ ";dialogHeight="+ h+ ";location=no;resizable=no;status=no;toolbar:no;scroll=yes;center=yes");
	}
	if (flag == 2) {
		window.showModalDialog("/mantoeye/terminal/terminal_brand_detail.jsp",obj,"dialogWidth="+ w+ ";dialogHeight="+ h+ ";location=no;resizable=no;status=no;toolbar:no;scroll=yes;center=yes");
	}
	if (flag == 3) {
		window.showModalDialog("/mantoeye/terminal/terminal_system_detail.jsp",obj,"dialogWidth="+ w+ ";dialogHeight="+ h+ ";location=no;resizable=no;status=no;toolbar:no;scroll=yes;center=yes");
	}
}

function renderInit(value ,record,columnObj,grid,colNo,rowNo){
	var vcTerminalBrand = record.vcTerminalBrand;
	var vcName = record.vcName;
	var nmAreaId = record.nmAreaId;
	var thtml = "<table><tr>";
	thtml = thtml +'<td><img style="cursor: hand" src="/skin/Default/images/MantoEye/icon/business.gif" alt="查看详细业务" onclick="openDistributeBusiness(1,\''+vcTerminalBrand+'\',\''+vcName+'\','+nmAreaId+');" /></td>';
	thtml = thtml +'</td><td></div><img style="cursor: hand" src="/skin/Default/images/MantoEye/icon/wap.gif" alt="查看终端品牌明细" onclick="openDistributeBusiness(2,\''+vcTerminalBrand+'\',\''+vcName+'\','+nmAreaId+');" /></td>';
	//thtml = thtml+ '</td><td></div><img style="cursor: hand" src="/skin/Default/images/MantoEye/icon/useragent.png" alt="查看系统类型明细" onclick="openDistributeBusiness(3,\''+vcTerminalBrand+'\',\''+vcName+'\');" /></td>';
...
var colsConfig = [
		...
		{ id : 'nmAreaId'      , header : "nmAreaId" ,hidden:true   },
...
var gridConfig={
	id : "datagrid",
	loadURL :'/terminalAreaAnlyse_query.do?1=1',
	exportURL :'/terminalAreaAnlyse_export.do?1=1' ,
	dataset : dsConfig ,
	columns : colsConfig ,
	width:780,
	height:500,
...
function buildChartData(totalRecords){
	...
	var title = time_search+" 流量|用户数品牌分析图";
