...
//var TerminalChangeId = url.substring(url.indexOf('=') + 1, url.length);
var  TerminalChangeId = url.substring(url.indexOf('=') + 1, url.indexOf('dataType')-1);
var dataType=url.substring(url.indexOf('dataType') + 9,url.length);
var date=new Date();
date=date.valueOf();
...
var dsConfig= {
	//data : data1 ,
	uniqueField : 'nmTerminalChangeIdTaskId' ,
	fields :[
		{name : 'nmTerminalChangeIdTaskId'        },
		{name : 'vcTaskName'        },
		{name : 'vcTaskOrder'        },
		{ name : 'dtOrderTime'       },
		{ name : 'dtTaskStartTime'       },
		{name : 'dtTaskEndTime'      },
...
function renderManager(value ,record,columnObj,grid,colNo,rowNo){
	var taskId = record.nmTerminalChangeIdTaskId;

	return '<table><tr>'
				+'<td><img style="cursor: hand" src="/skin/Default/images/MantoEye/icon/wap.png" alt="终端升级流量统计" onclick="openDistribute(1,\''+taskId+'\',\''+dataType+'\');" /></td>'
					+'<td><img style="cursor: hand" src="/skin/Default/images/MantoEye/icon/business.gif" alt="终端升级业务流量统计" onclick="openDistribute(2,\''+taskId+'\',\''+dataType+'\');" /></td>'
				+'</tr></table>'
}
function openDistribute(flag,taskId,dataType){
	var obj = new Object();
		obj.taskId = taskId;
		obj.dataType=dataType.substring(0,dataType.indexOf('&'));
if(flag==1)
		window.showModalDialog("/mantoeye/terminal/terminalUpgrade_Flush.jsp",obj,"dialogWidth="+ w+ ";dialogHeight="+ h+ ";location=no;resizable=no;status=no;toolbar:no;scroll=yes;center=yes");
if(flag==2)
	window.showModalDialog("/mantoeye/terminal/terminalUpgrade_BisFlush.jsp",obj,"dialogWidth="+ w+ ";dialogHeight="+ h+ ";location=no;resizable=no;status=no;toolbar:no;scroll=yes;center=yes");
}
...
var gridConfig={
	id : "datagrid",
	loadURL :'/terminalupgradetask_query.do?TerminalChangeId='+TerminalChangeId,
	exportURL :'/terminalupgradetask_export.do?1=1' ,             
	dataset : dsConfig ,
	columns : colsConfig ,
	width:780,
	height:500,
...
$(document).ready(function(){
	  ...
	  var bflags = initBtnPerm('_BTN_ADD,_BTN_EDIT,_BTN_DEL,_BTN_START,_BTN_PAUSE',permflag);
      var toolbar = new Toolbar({
        ...
        useable : bflags[0],
          handler : function(){
          		...
          		var v = showMMDialog('/terminalupgradetask_add.do?permId=${permId}&TerminalChangeId='+TerminalChangeId,'添加任务',width,height);
          		reloadData(v);
    ...
    useable : bflags[1],
          handler : function(){
          	...
          	if(count>1){
          		...
          		var v = showMMDialog('/terminalupgradetask_edit.do?keyid='+id,'编辑任务',width,height);
          		reloadData(v);
...
function deleteTask(){
	...
	if(flag){
		$.ajax({
			type : "post",
			url : "terminalupgradetask_deleteTask.do",
			data : {
				taskIds:taskIds
			},
...
function stopTask(){
	...
	if(count>1){
		...
		$.ajax({
			type : "post",
			url : "terminalupgradetask_stopTask.do",
			data : {
				taskId:taskId
			},
			success : function(msg) {
