...
#content_table_id  th {
	...
	<body style="overflow-x: hidden">
		<form name="beanView" action="/cameratrack_saveCameratrackApn.do"
								...
								<tr>
									...
									<td>
										<input type="text" id="apn" name="apn"
											value="${apn}" readonly onclick="showApnDialog()"
											style="width: 350px; height: 16px;" />
										<div style="height: 0px; border: 0px; overflow: hidden"
											id="apn_dialog_hidden_id"></div>
										<input id="apnIds" type="hidden" name="apnIds"
											value="${apnIds}" />
									</td>
...
function killErrors() {
	...
	var apnDialog = null;
	function showApnDialog(){
		if(apnDialog == null){
		  apnDialog = new SymbolDialog({
	        renderTo : 'apn',//点击触发对话框ID
	        renderTo2:'apn_dialog_hidden_id',
	        hiddenTo:'apnIds',//页面与对话框参数传递
			id:'apnDialog',//对话框ID
			...
			typeSearchName:'类型',
			modelSearchName:'APN',
			title:'APN复选对话框',//对话框标题
			url:'cameratrack_initApnDialogData.do'//对话框加载后台数据URL
	      });
		  apnDialog.init();//初始化页面DOM对象
		  apnDialog.loadData($('#apnIds').attr('value'));//加载后台数据并
		  apnDialog.display();//显示
		}else{
		  apnDialog.display();//显示(第N次以后只显示对话框即可,提高效率)
		}
		//隐藏select
