...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body style="overflow-x: hidden">
		<form name="beanForm" action="/sportcustomer_upFile.do"
					...
					<td width="100%">
						<div class="inputmain">
							...
							<table cellpadding="0" cellspacing="1" class="formitem">
								<tr>
									<td colspan="2" height="30px">
										<span style="color: #0000FF;">提示:输入规则为集团名称+号码，中间以逗号分隔。  不同号码以换行分隔，号码为不包含86的11位手机号码，文件类型应为dat结尾的txt类型的文件。</span>
									</td>
								</tr>
								<tr>
									<th>
							...
							<div class="dialog_msg_class"
								style="margin-top: 80px; margin-left: 70px;" id="showmsg"
...
function submitFrom() {
	if (document.getElementById("file").value == '') {
		...
		if ((fs.length < 2)
				|| ((fs[fs.length - 1] != 'dat') && (fs[fs.length - 1] != 'DAT'))) {
			alert("文件后缀名必须为dat");
		} else {
			showMsg();
