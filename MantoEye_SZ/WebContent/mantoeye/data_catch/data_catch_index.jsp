										...
										<td width="100%" class="condition_down">
											<table id="query_condition" cellspacing="0px"
												...
												<tr valign="middle">
													...
													<td class="condition_name" id="msis_td_head">
														号码：
													</td>
													<td class="condition_value" id="msis_td_value">
...
<script type="text/javascript">
/**
CATCH_HISBASE_DATA: 历史原始记录提取
CATCH_CURRBASE_DATA:实时原始数据提取
CATCH_HISANALYSIS_DATA:历史解析纪录提取
CATCH_IMSIS_DATA:用户号码提取
*/
//1.历史原始信令提取2.实时原始信令提取3.历史解析信令提取4、用户号码提取
var taskType=1 ; 
if(parent.tabId=='CATCH_HISBASE_DATA'){
	taskType = 1 ;
}else if(parent.tabId=='CATCH_CURRBASE_DATA'){
	taskType = 2 ;
}else if(parent.tabId=='CATCH_HISANALYSIS_DATA'){
	taskType = 3 ;
}else{
	taskType = 4;
}
...
function renderManager(value ,record,columnObj,grid,colNo,rowNo){
	var taskId = record.nmDataGetterTaskId;
	var taskType = record.intTaskType;
	var taskName = record.vcTaskName;
	var state = record.intTaskStatus;
	return '<table><tr>'
				+'<td><img style="cursor: hand" src="/skin/Default/images/MantoEye/icon/info.png" alt="查看任务" onclick="showDetail(\''+taskId+'\',\''+taskType+'\');" /></td>'
...
function showDetail(id,taskType){
	var width = '720px';
    var height = (checkIE()=='IE6')?'540px':'500px';
    if(taskType==4){
         height = (checkIE()=='IE6')?'600px':'580px';
    }
	showMMDialog('/dataCatch_show.do?keyid='+id+'&taskType='+taskType,taskType,width,height);
}
...
var colsConfig = [
	// isCheckColumn 来指定是不是"checkbox选择列" ,通过 grid.checkedRows 能取得所有被选中的行
		...
		{ id : 'nmTaskOrder'      , header : '定制人'   },
		{ id : 'msisdn'    , header : '号码'   },
		{ id : 'businessName'    , header : '业务', hidden:true   },
		{ id : 'areaName'    , header : '区域', hidden:true   },
...
var gridConfig={
	id : "datagrid",
	loadURL :'/dataCatch_query.do?taskType='+taskType,
	exportURL :'/dataCatch_export.do?1=1' ,             
	dataset : dsConfig ,
	columns : colsConfig ,
	width:780,
	height:500,
...
function loadComplate(){
	...
	if(taskType==4){
		obj.hideColumn = 2 ;//用户号码提取
	}
	obj.isCheckbox = true;//是否可选择
	obj.otherHeight = 0 ; //必须定义,因为有些页面不同，所以此处指定预留高度
	
	if(taskType==4){
		datagrid.getColumn('msisdn').hide();
		datagrid.getColumn('areaName').show();
		datagrid.getColumn('businessName').show();
	}
	//判断是否查询到数据
	judgeData(datagrid);
	//初始化grid
...
function query() {
	...
	//alert(orderTime_search+"^^^^"+orderTime_end);
	var param={
...
$(document).ready(function(){
	  ...
	  var bflags = initBtnPerm('_BTN_ADD,_BTN_EDIT,_BTN_PAUSE,_BTN_DEL',permflag);
	  var stopflag = bflags[2];
	  if(stopflag=='T'&&(taskType=='3'||taskType=='4')){
	  	stopflag='F';
	  }	  
      var toolbar = new Toolbar({
        ...
        items : [{
          ...
          handler : function(){
          		...
          		var v = showMMDialog('/dataCatch_add.do?permId=${permId }&taskType='+taskType,taskType,width,height);
          		reloadData(v);
          }
        },'-',{
          type : 'button',
          ...
          handler : function(){
          	...
          	if(count>1){
          		...
          		var v = showMMDialog('/dataCatch_edit.do?keyid='+id+'&taskType='+taskType,taskType,width,height);
          		reloadData(v);
          	}
          }
        },'-',{
...
function openDownload(taskId,taskType,state,taskName){
	if(state == 2||state == 3){
		...
		showMMDialog('/mantoeye/data_catch/data_catch_download.jsp',obj,'540px','420px');
	}else{
