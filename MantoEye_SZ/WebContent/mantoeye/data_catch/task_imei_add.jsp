...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	$(document).ready( function() {
		...
		//聚焦第一个输入框
			...
			jQuery.validator.addMethod("vcImei", function(value,element) {
				if(value.length==0){
					return true;
				}
					var reg0 = /^(\d{0,9}，)*(\d{0,9})$/;
					var reg1 = /^(\d{0,9},)*(\d{0,9})$/;
					var reg2 = /^(\d{0,9})$/;
					var flag = false;
					if (reg0.test(value)) {
						flag = true;
			...
			$("#beanForm").validate( {
				rules : {
					"vcImei" :{
						...
						remote:{
        					url:"dataOutput_checkUniquevcImei.do",
        					type:"post" ,
        					data:{"vcImei":function (){return $('#vcImei').attr('value')},
        							"taskId":obj.taskId
			...
			if(status=='1'){
				...
				$('#intImeiStatus1').attr('checked',true);	
				$('#intImeiStatus1').attr('disabled','disabled');
				$('#intImeiStatus2').attr('disabled','disabled');
			}
			if(status=='2'){
				$('#note').css({'display':'block'});
				$('#intImeiStatus2').attr('checked',true);	
				$('#intImeiStatus1').attr('disabled','disabled');
				$('#intImeiStatus2').attr('disabled','disabled');
			}*/
		});
		...
		function showDataType(value){
				if(value==1){
					$('#intImeiStatus').attr('value',1);	
				}
				if(value==2){
					$('#intImeiStatus').attr('value',2);	
				}
			}
</script>
		<style>
#content_table_id  th {
	...
	<body style="overflow-x: hidden">
		<form name="beanView" action="/dataOutput_savevcImei.do"
						...
						<div>
							...
							<table width="100%" cellpadding="0" cellspacing="1"
								...
								<tr>
									...
									<td>
										<input type="text" name="vcImei"
											style="width: 300px;" id="vcImei"
											value=""/>
										<input type="hidden" name='taskId' id="taskId"/>
										<input type="hidden" name='intImeiStatus' id="intImeiStatus" value="2"/>	
									</td>
								</tr>
								...
								<tr id="note" style="display: block">
									...
									<td >
										<font color="red">（IMEI匹配模式已经存在，若需要更改，请将已存在的IMEI删除！）</font>
									</td>
