...
$('#distribute_data_div').html("用户["+msisdn+"]TOP5 URL"+'分布数据');
$('#distribute_chart_div').html("用户["+msisdn+"]TOP5 URL"+'分布图形');
...
function loadComplate(){
	...
	if(typeof(datagrid.getRecord(0))!="undefined"){
			var chartxmldata;
						title=msisdn+' #FIELD#Top5URL占比图';
					chartxmldata='<root><chart id="1" name="'+title+'" fields="总流量|上流量|下流量">';
					for(var i = 0 ; i<totalRecords;i++){
						var record = datagrid.getRecord(i);
						chartxmldata=chartxmldata+'<data label="'+record.url+'" total="'+record.totalFlushMB+'"  imsis ="'+record.upFlushMB+'" count="'+record.downFlushMB+'"/>';
...
//	alert(othertotal+"-"+otherup+"-"+otherdown);
	if(typeof(datagrid.getRecord(0))!="undefined"){
		for(var i = 0 ; i<totalRecords;i++){	
			...
			datas1[i] = {"label":record.url,"value":record.totalFlushMB};
			datas2[i] = {"label":record.url,"value":record.upFlushMB};
			datas3[i] = {"label":record.url,"value":record.downFlushMB};
			}
		}
		if(morethan){
        ...
        items : [{
          ...
          handler : function(){
           //参数
			...
			fileObj.fileName='Top5URL分布数据';//('+startTime_search+'至'+endTime_search+')';//文件名称
