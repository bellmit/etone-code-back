...
$('#distribute_data_div').html("用户["+msisdn+"]TOP5 ContentType"+'分布数据');
$('#distribute_chart_div').html("用户["+msisdn+"]TOP5 ContentType"+'分布图形');
...
var colsConfig = [
		...
		{ id : 'contentType' , header : "ContentType"   ,renderer:rendererKillNull },
...
function loadComplate(){
	...
	if(typeof(datagrid.getRecord(0))!="undefined"){
			var chartxmldata;
					title=msisdn+' #FIELD#ContentType占比图';
					chartxmldata='<root><chart id="1" name="'+title+'" fields="总流量(MB)|上流量(MB)|下流量(MB)">';
					for(var i = 0 ; i<totalRecords;i++){
						var record = datagrid.getRecord(i);
						chartxmldata=chartxmldata+'<data label="'+record.contentType+'" total="'+record.totalFlushMB+'"  imsis ="'+record.upFlushMB+'" count="'+record.downFlushMB+'"/>';
...
//	alert(othertotal+"-"+otherup+"-"+otherdown);
	if(typeof(datagrid.getRecord(0))!="undefined"){
		for(var i = 0 ; i<totalRecords;i++){	
			var record = datagrid.getRecord(i);	 	
		 	...
		 	var chartlabel = buildNull(record.contentType);
	...
	if(!chartinit){
		...
		swfobject.embedSWF("/flash/open-flash-chart.swf?id=flush", "imgarea1", "33%", "100%", "9.0.0","expressInstall.swf",'',{"wmode":"opaque"});
		swfobject.embedSWF("/flash/open-flash-chart.swf?id=up", "imgarea2", "33%", "100%", "9.0.0","expressInstall.swf",'',{"wmode":"opaque"});
		swfobject.embedSWF("/flash/open-flash-chart.swf?id=down", "imgarea3", "33%", "100%", "9.0.0","expressInstall.swf",'',{"wmode":"opaque"});
	}else{
        ...
        items : [{
          ...
          handler : function(){
           //参数
			...
			fileObj.fileName='Top5contentType分布数据';//('+startTime_search+'至'+endTime_search+')';//文件名称
