...
$('#distribute_data_div').html("用户["+msisdn+"]TOP5 小时"+'分布数据');
$('#distribute_chart_div').html("用户["+msisdn+"]TOP5 小时"+'分布图形');
...
function loadComplate(){
	...
	if(typeof(datagrid.getRecord(0))!="undefined"){
			var chartxmldata;
					title=msisdn+' #FIELD#Top5时段占比图';
					chartxmldata='<root><chart id="1" name="'+title+'" fields="总流量(MB)|上流量(MB)|下流量(MB)">';
					for(var i = 0 ; i<totalRecords;i++){
						var record = datagrid.getRecord(i);
						chartxmldata=chartxmldata+'<data label="'+record.hour+'" total="'+record.totalFlushMB+'"  imsis ="'+record.upFlushMB+'" count="'+record.downFlushMB+'"/>';
...
//	alert(othertotal+"-"+otherup+"-"+otherdown);
	if(typeof(datagrid.getRecord(0))!="undefined"){
		for(var i = 0 ; i<totalRecords;i++){	
			...
			datas1[i] = {"label":record.hour,"value":record.totalFlushMB};
			datas2[i] = {"label":record.hour,"value":record.upFlushMB};
			datas3[i] = {"label":record.hour,"value":record.downFlushMB};
			}
		}
		if(morethan){
        ...
        items : [{
          ...
          handler : function(){
           //参数
			...
			fileObj.fileName='Top5小时点分布数据';//('+startTime_search+'至'+endTime_search+')';//文件名称
