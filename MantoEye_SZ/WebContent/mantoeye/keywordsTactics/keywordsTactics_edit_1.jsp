		...
		//聚焦第一个输入框
			...
			$("#beanForm").validate( {
				...
				submitHandler: function(form) {  
					...
					var vcFilterKeyValue1 = $('#vcFilterKeyValue1').attr("value");
					var vcFilterKeyValue2 = $('#vcFilterKeyValue2').attr("value");	
					if(vcFilterKeyValue1=="" && vcFilterKeyValue2==""){
						alert("关键字不能为空！")
						return;
					}
					/*vcFilterKeyValue1 = vcFilterKeyValue1.replace("，",",");
					vcFilterKeyValue2 = vcFilterKeyValue2.replace("，",",");
					var key1=vcFilterKeyValue1.split(",");
					var key2=vcFilterKeyValue2.split(",");
					if(vcFilterKeyValue1.indexOf("飞信")>-1 && key1.length>5){
						alert("定制飞信业务的关键字，请不要超过5个")
						return;
					}
					if(vcFilterKeyValue2.indexOf("飞信")>-1 && key2.length>5){
						alert("定制飞信业务的关键字，请不要超过5个")
						return;
					}*/
...
#content_table_id  th {
	...
	<body style="overflow-x: hidden">
		<form name="beanView" action="/modules/keywordsTactics/keywords-tactics!update.do"
						...
						<div>
							...
							<table width="100%" cellpadding="0" cellspacing="1"
								...
								<tr>
									...
									<td>
										<input type="text" name="entity.vcTaskName" disabled="disabled"
											style="width: 350px;" id="vcTaskName"
											value="${entity.vcTaskName}" />
										<input type="hidden" id="nmDataGetterTaskId" name="entity.nmDataGetterTaskId" value="${entity.nmDataGetterTaskId}"/>
										<input type="hidden" id="vcTaskOrder" name="entity.vcTaskOrder" value="${entity.vcTaskOrder}"/>
										<input type="hidden" id="dtOrderTime" name="entity.dtOrderTime" value="${entity.dtOrderTime}"/>
										<input type="hidden" id="bitTaskActiveFlag" name="entity.bitTaskActiveFlag" value="${entity.bitTaskActiveFlag}"/>
										<input type="hidden" id="intTaskStatus" name="entity.intTaskStatus" value="${entity.intTaskStatus}"/>
										<input type="hidden" id="intTaskType" name="entity.intTaskType" value="${entity.intTaskType}"/>
									</td>
								</tr>
						...
						<select id="intEndHour" name="entity.intEndHour" style="width:162px" onchange="changEndHour()" value="${entity.intEndHour}">
							...
							<tr>
								...
								<tr>
									...
									<td >
										<input type="text" name="vcFilterKeyValue1"
											style="width: 350px;" id="vcFilterKeyValue1" value="${vcFilterKeyValue1}"
											 />
									</td>
								...
								<tr>
									...
									<td >
										<input type="text" name="vcFilterKeyValue2"
											style="width: 350px;" id="vcFilterKeyValue2" value="${vcFilterKeyValue2}"
											 />
									</td>
								...
								<tr>
									...
									<td >
										<font color="red">提示：多个关键字可以用“，”分隔。只支持20~40个自定义搜索关键字筛选。</font>
									</td>
...
<script>
document.getElementById('intStartHour').value = "<c:out value='${entity.intStartHour}'/>";
document.getElementById('intEndHour').value = "<c:out value='${entity.intEndHour}'/>";
var date=new Date();
	...
	function showBusinessDialog(){
		if(businessDialog == null){
		  businessDialog = new SymbolDialog({
			...
			typeSearchName:'引擎',
			modelSearchName:'内容',
			title:'搜索引擎复选对话框',//对话框标题
			url:'/modules/keywordsTactics/keywords-tactics!initBussnessDialogData.do?1=1'//对话框加载后台数据URL
	      });
