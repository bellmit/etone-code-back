...
/**
 * z.z.w.project.dbop.FSSybaseIqOpertor.java
 */
public class FSSybaseIqOpertor extends CacheDBFactory implements Runnable {
		...
		try {
			...
			while (it.hasNext()) {
				try {
					...
					String sql = "update epmp_tbservicerule_bak set vcIp = '"
	...
	protected boolean getCache() {
		...
		try {
			...
			LogTools.getLogger(getClass()).debug(
					"Select epmp_tbServiceRule complete, dataList size :[{}].",
					list.size());
			return true;
		} catch (Exception e) {
			LogTools.getLogger(getClass())
					.error("When [{} - {} --> Exception ] ,  \n  MASSAGE : {} \n  CAUSE :{} \n  CLASS : {}",
							new Object[] { "select tbdistrict & tbtown",
									...
									e.getClass() });
			LogTools.getLogger(getClass()).warn(
					"Select epmp_tbServiceRule cache,return false!");
			return false;
		} finally {
			clearWork();
		}
